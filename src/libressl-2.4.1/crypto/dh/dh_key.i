...
# 123 "dh/dh_key.c"
  ...
  if (dh->q) {
   do {
    if (!BN_rand_range(priv_key, dh->q))
     goto err;
   } while (BN_is_zero(priv_key) || ((((((priv_key))->top == 1) && (((priv_key))->d[0] == (unsigned long)(1))) || (((1) == 0) && (((priv_key))->top == 0))) && !(priv_key)->neg));
  } else {
...

 {
  ...
  if ((dh->flags & 0x02) == 0) {
   BN_init(&local_prk);
   prk = &local_prk;
   ((prk)->d=(priv_key)->d, (prk)->top=(priv_key)->top, (prk)->dmax=(priv_key)->dmax, (prk)->neg=(priv_key)->neg, (prk)->flags=(((prk)->flags & 0x01) | ((priv_key)->flags & ~0x01) | 0x02 | (0x04)));
  } else
   prk = priv_key;
...
err:
 if (ok != 1)
  ERR_put_error(5,(103),(3),"dh/dh_key.c",170);
...
# 184 "dh/dh_key.c"
 ...
 if (BN_num_bits(dh->p) > 10000) {
  ERR_put_error(5,(102),(103),"dh/dh_key.c",190);
  goto err;
 }
...
# 201 "dh/dh_key.c"
  ...
  ERR_put_error(5,(102),(100),"dh/dh_key.c",202);
  goto err;
 }
 ...
 if (dh->flags & 0x01) {
  ...
  ERR_put_error(5,(102),(102),"dh/dh_key.c",218);
  goto err;
 }

 if (!dh->meth->bn_mod_exp(dh, tmp, pub_key, dh->priv_key, dh->p, ctx,
     mont)) {
  ERR_put_error(5,(102),(3),"dh/dh_key.c",224);
  goto err;
 }
