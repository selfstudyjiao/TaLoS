...
void ERR_load_TS_strings(void);
# 66 "ts/ts_rsp_verify.c" 2



static int TS_verify_cert(X509_STORE *store, struct stack_st_X509 *untrusted,
    X509 *signer, struct stack_st_X509 **chain);
static int TS_check_signing_certs(PKCS7_SIGNER_INFO *si, struct stack_st_X509 *chain);
static ESS_SIGNING_CERT *ESS_get_signing_cert(PKCS7_SIGNER_INFO *si);
static int TS_find_cert(struct stack_st_ESS_CERT_ID *cert_ids, X509 *cert);
static int TS_issuer_serial_cmp(ESS_ISSUER_SERIAL *is, X509_CINF *cinfo);
static int int_TS_RESP_verify_token(TS_VERIFY_CTX *ctx,
    PKCS7 *token, TS_TST_INFO *tst_info);
static int TS_check_status_info(TS_RESP *response);
static char *TS_get_status_text(struct stack_st_ASN1_UTF8STRING *text);
...
static int TS_find_name(struct stack_st_GENERAL_NAME *gen_names, GENERAL_NAME *name);
...
# 143 "ts/ts_rsp_verify.c"
int
TS_RESP_verify_signature(PKCS7 *token, struct stack_st_X509 *certs,
    X509_STORE *store, X509 **signer_out)
{
 struct stack_st_PKCS7_SIGNER_INFO *sinfos = 
# 147 "ts/ts_rsp_verify.c" 3 4
                                      ((void *)0)
# 147 "ts/ts_rsp_verify.c"
                                          ;
 PKCS7_SIGNER_INFO *si;
 struct stack_st_X509 *signers = 
# 149 "ts/ts_rsp_verify.c" 3 4
                          ((void *)0)
# 149 "ts/ts_rsp_verify.c"
                              ;
 X509 *signer;
 struct stack_st_X509 *chain = 
# 151 "ts/ts_rsp_verify.c" 3 4
                        ((void *)0)
# 151 "ts/ts_rsp_verify.c"
                            ;
 char buf[4096];
 int i, j = 0, ret = 0;
 BIO *p7bio = 
# 154 "ts/ts_rsp_verify.c" 3 4
             ((void *)0)
# 154 "ts/ts_rsp_verify.c"
                 ;


 if (!token) {
  ERR_put_error(47,(106),(102),"ts/ts_rsp_verify.c",158);
  goto err;
 }


 if (!(OBJ_obj2nid((token)->type) == 22)) {
  ERR_put_error(47,(106),(114),"ts/ts_rsp_verify.c",164);
  goto err;
 }


 sinfos = PKCS7_get_signer_info(token);
 if (!sinfos || sk_num(((_STACK*) (1 ? (sinfos) : (struct stack_st_PKCS7_SIGNER_INFO*)0))) != 1) {
  ERR_put_error(47,(106),(110),"ts/ts_rsp_verify.c",172)
                                    ;
  goto err;
 }
 si = ((PKCS7_SIGNER_INFO *)sk_value(((_STACK*) (1 ? (sinfos) : (struct stack_st_PKCS7_SIGNER_INFO*)0)), (0)));


 if (PKCS7_ctrl(token,2,0,
# 178 "ts/ts_rsp_verify.c" 3 4
    ((void *)0)
# 178 "ts/ts_rsp_verify.c"
    )) {
  ERR_put_error(47,(106),(106),"ts/ts_rsp_verify.c",179);
  goto err;
 }
 ...
 if (!signers || sk_num(((_STACK*) (1 ? (signers) : (struct stack_st_X509*)0))) != 1)
  goto err;
 signer = ((X509 *)sk_value(((_STACK*) (1 ? (signers) : (struct stack_st_X509*)0)), (0)));
 ...
 p7bio = PKCS7_dataInit(token, 
# 200 "ts/ts_rsp_verify.c" 3 4
                              ((void *)0)
# 200 "ts/ts_rsp_verify.c"
                                  );
 ...
 if (j <= 0) {
  ERR_put_error(47,(106),(109),"ts/ts_rsp_verify.c",209);
  goto err;
 }
 ...
 if (signer_out) {
  *signer_out = signer;
  CRYPTO_add_lock(&signer->references,1,3,"ts/ts_rsp_verify.c",216);
 }
...
err:
 ...
 sk_free(((_STACK*) (1 ? (signers) : (struct stack_st_X509*)0)));
...
static int
TS_verify_cert(X509_STORE *store, struct stack_st_X509 *untrusted, X509 *signer,
    struct stack_st_X509 **chain)
{
 ...
 *chain = 
# 242 "ts/ts_rsp_verify.c" 3 4
         ((void *)0)
# 242 "ts/ts_rsp_verify.c"
             ;
 if (X509_STORE_CTX_init(&cert_ctx, store, signer, untrusted) == 0) {
  ERR_put_error(47,(109),(11),"ts/ts_rsp_verify.c",244);
  goto err;
 }
 X509_STORE_CTX_set_purpose(&cert_ctx, 9);
 i = X509_verify_cert(&cert_ctx);
 if (i <= 0) {
  int j = X509_STORE_CTX_get_error(&cert_ctx);

  ERR_put_error(47,(109),(100),"ts/ts_rsp_verify.c",252);
...
static int
TS_check_signing_certs(PKCS7_SIGNER_INFO *si, struct stack_st_X509 *chain)
{
 ESS_SIGNING_CERT *ss = ESS_get_signing_cert(si);
 struct stack_st_ESS_CERT_ID *cert_ids = 
# 272 "ts/ts_rsp_verify.c" 3 4
                                  ((void *)0)
# 272 "ts/ts_rsp_verify.c"
                                      ;
 X509 *cert;
 int i = 0;
 int ret = 0;
 ...
 cert = ((X509 *)sk_value(((_STACK*) (1 ? (chain) : (struct stack_st_X509*)0)), (0)));
 if (TS_find_cert(cert_ids, cert) != 0)
  goto err;



 if (sk_num(((_STACK*) (1 ? (cert_ids) : (struct stack_st_ESS_CERT_ID*)0))) > 1) {

  for (i = 1; i < sk_num(((_STACK*) (1 ? (chain) : (struct stack_st_X509*)0))); ++i) {
   cert = ((X509 *)sk_value(((_STACK*) (1 ? (chain) : (struct stack_st_X509*)0)), (i)));
   if (TS_find_cert(cert_ids, cert) < 0)
    goto err;
  }
 }
...
err:
 if (!ret)
  ERR_put_error(47,(103),(101),"ts/ts_rsp_verify.c",300)
                                         ;
...
ESS_get_signing_cert(PKCS7_SIGNER_INFO *si)
{
 ...
 if (!attr)
  return 
# 314 "ts/ts_rsp_verify.c" 3 4
        ((void *)0)
# 314 "ts/ts_rsp_verify.c"
            ;
 if (attr->type != 16)
  return 
# 316 "ts/ts_rsp_verify.c" 3 4
        ((void *)0)
# 316 "ts/ts_rsp_verify.c"
            ;
 p = attr->value.sequence->data;
 return d2i_ESS_SIGNING_CERT(
# 318 "ts/ts_rsp_verify.c" 3 4
                            ((void *)0)
# 318 "ts/ts_rsp_verify.c"
                                , &p, attr->value.sequence->length);
}


static int
TS_find_cert(struct stack_st_ESS_CERT_ID *cert_ids, X509 *cert)
{
 int i;
 ...
 for (i = 0; i < sk_num(((_STACK*) (1 ? (cert_ids) : (struct stack_st_ESS_CERT_ID*)0))); ++i) {
  ESS_CERT_ID *cid = ((ESS_CERT_ID *)sk_value(((_STACK*) (1 ? (cert_ids) : (struct stack_st_ESS_CERT_ID*)0)), (i)));
...
TS_issuer_serial_cmp(ESS_ISSUER_SERIAL *is, X509_CINF *cinfo)
{
 ...
 if (!is || !cinfo || sk_num(((_STACK*) (1 ? (is->issuer) : (struct stack_st_GENERAL_NAME*)0))) != 1)
  return -1;


 issuer = ((GENERAL_NAME *)sk_value(((_STACK*) (1 ? (is->issuer) : (struct stack_st_GENERAL_NAME*)0)), (0)));
 if (issuer->type != 4 ||
...
# 428 "ts/ts_rsp_verify.c"
static int
int_TS_RESP_verify_token(TS_VERIFY_CTX *ctx, PKCS7 *token,
    TS_TST_INFO *tst_info)
{
 X509 *signer = 
# 432 "ts/ts_rsp_verify.c" 3 4
               ((void *)0)
# 432 "ts/ts_rsp_verify.c"
                   ;
 GENERAL_NAME *tsa_name = TS_TST_INFO_get_tsa(tst_info);
 X509_ALGOR *md_alg = 
# 434 "ts/ts_rsp_verify.c" 3 4
                     ((void *)0)
# 434 "ts/ts_rsp_verify.c"
                         ;
 unsigned char *imprint = 
# 435 "ts/ts_rsp_verify.c" 3 4
                         ((void *)0)
# 435 "ts/ts_rsp_verify.c"
                             ;
 ...
 if ((ctx->flags & (1u << 1)) &&
  ...
  ERR_put_error(47,(149),(113),"ts/ts_rsp_verify.c",447);
  goto err;
 }
 ...
 if ((ctx->flags & (1u << 6)) &&
  ...
  ERR_put_error(47,(149),(111),"ts/ts_rsp_verify.c",477);
  goto err;
 }
 ...
 if ((ctx->flags & (1u << 7)) &&
  ...
  ERR_put_error(47,(149),(112),"ts/ts_rsp_verify.c",484);
  goto err;
 }
...
# 502 "ts/ts_rsp_verify.c" 3 4
                          ((void *)0)
# 502 "ts/ts_rsp_verify.c"
                              ;
 char *embedded_status_text = 
# 503 "ts/ts_rsp_verify.c" 3 4
                             ((void *)0)
# 503 "ts/ts_rsp_verify.c"
                                 ;
 char failure_text[256] = "";
 ...
 if (0 <= status && status < (long)(sizeof(TS_status_text)/sizeof(*TS_status_text)))
  status_text = TS_status_text[status];
 else
  status_text = "unknown code";


 if (sk_num(((_STACK*) (1 ? (info->text) : (struct stack_st_ASN1_UTF8STRING*)0))) > 0 &&
 ...
 if (info->failure_info) {
  int i;
  int first = 1;
  for (i = 0; i < (int)(sizeof(TS_failure_info) / sizeof(*TS_failure_info)); ++i) {
 ...
 if (failure_text[0] == '\0')
  strlcpy(failure_text, "unspecified", 256);


 ERR_put_error(47,(104),(107),"ts/ts_rsp_verify.c",542);
...
static char *
TS_get_status_text(struct stack_st_ASN1_UTF8STRING *text)
{
 int i;
 unsigned int length = 0;
 char *result = 
# 558 "ts/ts_rsp_verify.c" 3 4
               ((void *)0)
# 558 "ts/ts_rsp_verify.c"
                   ;


 for (i = 0; i < sk_num(((_STACK*) (1 ? (text) : (struct stack_st_ASN1_UTF8STRING*)0))); ++i) {
  ASN1_UTF8STRING *current = ((ASN1_UTF8STRING *)sk_value(((_STACK*) (1 ? (text) : (struct stack_st_ASN1_UTF8STRING*)0)), (i)));
  length += ASN1_STRING_length(current);
  length += 1;
 }

 if (!(result = malloc(length))) {
  ERR_put_error(47,(105),((1|64)),"ts/ts_rsp_verify.c",568);
  return 
# 569 "ts/ts_rsp_verify.c" 3 4
        ((void *)0)
# 569 "ts/ts_rsp_verify.c"
            ;
 }
...
TS_check_policy(ASN1_OBJECT *req_oid, TS_TST_INFO *tst_info)
{
 ...
 if (OBJ_cmp(req_oid, resp_oid) != 0) {
  ERR_put_error(47,(102),(108),"ts/ts_rsp_verify.c",588);
  return 0;
 }
 ...
 *md_alg = 
# 606 "ts/ts_rsp_verify.c" 3 4
          ((void *)0)
# 606 "ts/ts_rsp_verify.c"
              ;
 *imprint = 
# 607 "ts/ts_rsp_verify.c" 3 4
           ((void *)0)
# 607 "ts/ts_rsp_verify.c"
               ;
 ...
 if (!(md = EVP_get_digestbyname(OBJ_nid2sn(OBJ_obj2nid((*md_alg)->algorithm))))) {
  ERR_put_error(47,(145),(126),"ts/ts_rsp_verify.c",615);
  goto err;
 }
 ...
 *imprint_len = length;
 if (!(*imprint = malloc(*imprint_len))) {
  ERR_put_error(47,(145),((1|64)),"ts/ts_rsp_verify.c",625);
  goto err;
 }
 ...
 if (!EVP_DigestFinal(&md_ctx, *imprint, 
# 635 "ts/ts_rsp_verify.c" 3 4
                                        ((void *)0)
# 635 "ts/ts_rsp_verify.c"
                                            ))
  goto err;
...
err:
 ...
 free(*imprint);
 *imprint = 
# 643 "ts/ts_rsp_verify.c" 3 4
           ((void *)0)
# 643 "ts/ts_rsp_verify.c"
               ;
 *imprint_len = 0;
 return 0;
}
    ...
    TS_TST_INFO *tst_info)
{
 ...
 if (algor_a) {
  ...
  if ((algor_a->parameter &&
      ASN1_TYPE_get(algor_a->parameter) != 5) ||
      (algor_b->parameter &&
      ASN1_TYPE_get(algor_b->parameter) != 5))
   goto err;
 }
...
err:
 if (!ret)
  ERR_put_error(47,(100),(103),"ts/ts_rsp_verify.c",676);
 return ret;
}
...
TS_check_nonces(const ASN1_INTEGER *a, TS_TST_INFO *tst_info)
{
 ...
 if (!b) {
  ERR_put_error(47,(101),(105),"ts/ts_rsp_verify.c",687);
  return 0;
 }


 if (ASN1_INTEGER_cmp(a, b) != 0) {
  ERR_put_error(47,(101),(104),"ts/ts_rsp_verify.c",693);
  return 0;
 }
...
{
 struct stack_st_GENERAL_NAME *gen_names = 
# 705 "ts/ts_rsp_verify.c" 3 4
                                    ((void *)0)
# 705 "ts/ts_rsp_verify.c"
                                        ;
 int idx = -1;
 int found = 0;

 if (signer == 
# 709 "ts/ts_rsp_verify.c" 3 4
              ((void *)0)
# 709 "ts/ts_rsp_verify.c"
                  )
  return 0;
 ...
 gen_names = X509_get_ext_d2i(signer, 85,
     
# 719 "ts/ts_rsp_verify.c" 3 4
    ((void *)0)
# 719 "ts/ts_rsp_verify.c"
        , &idx);
 while (gen_names != 
# 720 "ts/ts_rsp_verify.c" 3 4
                    ((void *)0) 
# 720 "ts/ts_rsp_verify.c"
                         &&
...
# 726 "ts/ts_rsp_verify.c" 3 4
     ((void *)0)
# 726 "ts/ts_rsp_verify.c"
         , &idx);
 }
 if (gen_names)
  GENERAL_NAMES_free(gen_names);
...
static int
TS_find_name(struct stack_st_GENERAL_NAME *gen_names, GENERAL_NAME *name)
{
 int i, found;
 for (i = 0, found = 0; !found && i < sk_num(((_STACK*) (1 ? (gen_names) : (struct stack_st_GENERAL_NAME*)0)));
     ++i) {
  GENERAL_NAME *current = ((GENERAL_NAME *)sk_value(((_STACK*) (1 ? (gen_names) : (struct stack_st_GENERAL_NAME*)0)), (i)));
