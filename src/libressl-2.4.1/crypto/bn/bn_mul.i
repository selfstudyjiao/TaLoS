...
# 83 "bn/bn_mul.c"
unsigned long
bn_sub_part_words(unsigned long *r, const unsigned long *a, const unsigned long *b, int cl,
    int dl)
{
 unsigned long c, t;
...
unsigned long
bn_add_part_words(unsigned long *r, const unsigned long *a, const unsigned long *b, int cl,
    int dl)
{
 unsigned long c, l, t;
...
# 237 "bn/bn_mul.c"
 ...
 if (dl < 0) {
  ...
  while (c) {
   l = (c + b[0]) & (0xffffffffffffffffL);
   c = (l < c);
   r[0] = l;
   if (++dl >= 0)
    break;

   l = (c + b[1]) & (0xffffffffffffffffL);
   c = (l < c);
   r[1] = l;
   if (++dl >= 0)
    break;

   l = (c + b[2]) & (0xffffffffffffffffL);
   c = (l < c);
   r[2] = l;
   if (++dl >= 0)
    break;

   l = (c + b[3]) & (0xffffffffffffffffL);
   c = (l < c);
   r[3] = l;
   if (++dl >= 0)
    break;
  ...
  while (c) {
   t = (a[0] + c) & (0xffffffffffffffffL);
   c = (t < c);
   r[0] = t;
   if (--dl <= 0)
    break;

   t = (a[1] + c) & (0xffffffffffffffffL);
   c = (t < c);
   r[1] = t;
   if (--dl <= 0)
    break;

   t = (a[2] + c) & (0xffffffffffffffffL);
   c = (t < c);
   r[2] = t;
   if (--dl <= 0)
    break;

   t = (a[3] + c) & (0xffffffffffffffffL);
   c = (t < c);
   r[3] = t;
   if (--dl <= 0)
    break;
...
# 434 "bn/bn_mul.c"
void
bn_mul_recursive(unsigned long *r, unsigned long *a, unsigned long *b, int n2, int dna,
    int dnb, unsigned long *t)
{
 int n = n2 / 2, c1, c2;
 int tna = n + dna, tnb = n + dnb;
 unsigned int neg, zero;
 unsigned long ln, lo, *p;
# 456 "bn/bn_mul.c"
 if (n2 == 8 && dna == 0 && dnb == 0) {
  bn_mul_comba8(r, a, b);
  return;
...

 {
  if (!zero)
   ...
   memset(&(t[n2]), 0, 8 * sizeof(unsigned long));
...

 {
  if (!zero)
   ...
   memset(&(t[n2]), 0, 16 * sizeof(unsigned long));
...

 {
  ...
  if (!zero)
   ...
   memset(&(t[n2]), 0, n2 * sizeof(unsigned long));
 ...
 if (c1) {
  p = &(r[n + n2]);
  lo= *p;
  ln = (lo + c1) & (0xffffffffffffffffL);
  *p = ln;



  if (ln < (unsigned long)c1) {
   do {
    p++;
    lo= *p;
    ln = (lo + 1) & (0xffffffffffffffffL);
    *p = ln;
   } while (ln == 0);
  }
 }
...
void
bn_mul_part_recursive(unsigned long *r, unsigned long *a, unsigned long *b, int n, int tna,
    int tnb, unsigned long *t)
{
 int i, j, n2 = n * 2;
 int c1, c2, neg;
 unsigned long ln, lo, *p;
...
# 641 "bn/bn_mul.c"
  ...
  memset(&(r[n2 + tna + tnb]), 0,
      sizeof(unsigned long) * (n2 - tna - tnb));
 } else {
  p = &(t[n2*2]);
  bn_mul_recursive(&(t[n2]), t, &(t[n]), n, 0, 0, p);
  bn_mul_recursive(r, a, b, n, 0, 0, p);
  ...
  {
   memset(&(r[n2]), 0, sizeof(unsigned long) * n2);
   if (tna < (16) &&
       tnb < (16)) {
...
void
bn_mul_low_recursive(unsigned long *r, unsigned long *a, unsigned long *b, int n2, unsigned long *t)
{
 int n = n2 / 2;
...
void
bn_mul_high(unsigned long *r, unsigned long *a, unsigned long *b, unsigned long *l, int n2,
    unsigned long *t)
{
 int i, n;
 int c1, c2;
 int neg, oneg, zero;
 unsigned long ll, lc, *lp, *mp;
...
# 856 "bn/bn_mul.c"
  ...
  mp = &(t[n2]);
  for (i = 0; i < n; i++)
   lp[i] = ((~mp[i]) + 1) & (0xffffffffffffffffL);
 }
# 878 "bn/bn_mul.c"
 if (l != 
# 878 "bn/bn_mul.c" 3 4
...
# 992 "bn/bn_mul.c"
  if (al == 8) {
   if ((((16) <= (rr)->dmax)?(rr):bn_expand2((rr),(16))) == 
# 993 "bn/bn_mul.c" 3 4
                            ((void *)0)
# 993 "bn/bn_mul.c"
                                )
 ...
 if ((al >= (16)) && (bl >= (16))) {
  if (i >= -1 && i <= 1) {
   ...
   if (i >= 0) {
    j = BN_num_bits_word((unsigned long)al);
   }
   if (i == -1) {
    j = BN_num_bits_word((unsigned long)bl);
   }
   j = 1 << (j - 1);
   
# 1013 "bn/bn_mul.c" 3 4
  ((void) (0))
# 1013 "bn/bn_mul.c"
                            ;
   k = j + j;
   if ((t = BN_CTX_get(ctx)) == 
# 1015 "bn/bn_mul.c" 3 4
                               ((void *)0)
# 1015 "bn/bn_mul.c"
                                   )
    goto err;
   if (al > j || bl > j) {
    if ((((k * 4) <= (t)->dmax)?(t):bn_expand2((t),(k * 4))) == 
# 1018 "bn/bn_mul.c" 3 4
                               ((void *)0)
# 1018 "bn/bn_mul.c"
                                   )
     goto err;
    if ((((k * 4) <= (rr)->dmax)?(rr):bn_expand2((rr),(k * 4))) == 
# 1020 "bn/bn_mul.c" 3 4
                                ((void *)0)
# 1020 "bn/bn_mul.c"
                                    )
     goto err;
   ...
   {
    if ((((k * 2) <= (t)->dmax)?(t):bn_expand2((t),(k * 2))) == 
# 1027 "bn/bn_mul.c" 3 4
                               ((void *)0)
# 1027 "bn/bn_mul.c"
                                   )
     goto err;
    if ((((k * 2) <= (rr)->dmax)?(rr):bn_expand2((rr),(k * 2))) == 
# 1029 "bn/bn_mul.c" 3 4
                                ((void *)0)
# 1029 "bn/bn_mul.c"
                                    )
     goto err;
...
# 1080 "bn/bn_mul.c"
 ...
 if ((((top) <= (rr)->dmax)?(rr):bn_expand2((rr),(top))) == 
# 1082 "bn/bn_mul.c" 3 4
                           ((void *)0)
# 1082 "bn/bn_mul.c"
                               )
  goto err;
 ...
 { unsigned long *ftl; int tmp_top = (rr)->top; if (tmp_top > 0) { for (ftl= &((rr)->d[tmp_top-1]); tmp_top > 0; tmp_top--) if (*(ftl--)) break; (rr)->top = tmp_top; } ; };
 if (r != rr)
  BN_copy(r, rr);
 ret = 1;
err:
...
void
bn_mul_normal(unsigned long *r, unsigned long *a, int na, unsigned long *b, int nb)
{
 unsigned long *rr;
...
void
bn_mul_low_normal(unsigned long *r, unsigned long *a, unsigned long *b, int n)
{
