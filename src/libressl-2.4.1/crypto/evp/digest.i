...
{
 EVP_MD_CTX_init(ctx);
 return EVP_DigestInit_ex(ctx, type, 
# 146 "evp/digest.c" 3 4
                                    ((void *)0)
# 146 "evp/digest.c"
                                        );
}
...
EVP_DigestInit_ex(EVP_MD_CTX *ctx, const EVP_MD *type, ENGINE *impl)
{
 ...
 if (type) {
  ...
  if (impl) {
   if (!ENGINE_init(impl)) {
    ERR_put_error(6,(128),(134),"evp/digest.c",176)
                                   ;
    return 0;
   }
  } else
  ...
  if (impl) {
   ...
   if (!d) {
    ...
    ERR_put_error(6,(128),(134),"evp/digest.c",188)
                                   ;
    ENGINE_finish(impl);
    return 0;
   }
   ...
   ctx->engine = 
# 199 "evp/digest.c" 3 4
                ((void *)0)
# 199 "evp/digest.c"
                    ;
 } else if (!ctx->digest) {
  ERR_put_error(6,(128),(139),"evp/digest.c",201);
  return 0;
 }

 if (ctx->digest != type) {
  if (ctx->digest && ctx->digest->ctx_size && ctx->md_data &&
      !EVP_MD_CTX_test_flags(ctx, 0x0004)) {
   explicit_bzero(ctx->md_data, ctx->digest->ctx_size);
   free(ctx->md_data);
   ctx->md_data = 
# 210 "evp/digest.c" 3 4
                 ((void *)0)
# 210 "evp/digest.c"
                     ;
  }
  ctx->digest = type;
  if (!(ctx->flags & 0x0100) && type->ctx_size) {
   ctx->update = type->update;
   ctx->md_data = malloc(type->ctx_size);
   if (ctx->md_data == 
# 216 "evp/digest.c" 3 4
                      ((void *)0)
# 216 "evp/digest.c"
                          ) {
    EVP_PKEY_CTX_free(ctx->pctx);
    ctx->pctx = 
# 218 "evp/digest.c" 3 4
               ((void *)0)
# 218 "evp/digest.c"
                   ;
    ERR_put_error(6,(128),((1|64)),"evp/digest.c",220)
                             ;
    return 0;
   }
  }
...
skip_to_init:
 ...
 if (ctx->pctx) {
  int r;
  r = EVP_PKEY_CTX_ctrl(ctx->pctx, -1, ((1<<3) | (1<<4) | (1<<5) | (1<<6) | (1<<7)),
      7, 0, ctx);
  if (r <= 0 && (r != -2))
   return 0;
 }
...
EVP_DigestFinal_ex(EVP_MD_CTX *ctx, unsigned char *md, unsigned int *size)
{
 ...
 if ((size_t)ctx->digest->md_size > 64) {
  ERR_put_error(6,(196),(164),"evp/digest.c",275);
  return 0;
 }
 ret = ctx->digest->final(ctx, md);
 if (size != 
# 279 "evp/digest.c" 3 4
            ((void *)0)
# 279 "evp/digest.c"
                )
 ...
 if (ctx->digest->cleanup) {
  ...
  EVP_MD_CTX_set_flags(ctx, 0x0002);
 }
...
# 301 "evp/digest.c"
  ...
  ERR_put_error(6,(110),(111),"evp/digest.c",302);
  return 0;
 }


 if (in->engine && !ENGINE_init(in->engine)) {
  ERR_put_error(6,(110),(38),"evp/digest.c",308);
  return 0;
 }
 ...
 if (out->digest == in->digest) {
  tmp_buf = out->md_data;
  EVP_MD_CTX_set_flags(out, 0x0004);
 } else
  tmp_buf = 
# 317 "evp/digest.c" 3 4
           ((void *)0)
# 317 "evp/digest.c"
               ;
 ...
 if (in->md_data && out->digest->ctx_size) {
  if (tmp_buf)
   ...
   if (!out->md_data) {
    ERR_put_error(6,(110),((1|64)),"evp/digest.c",328)
                             ;
    return 0;
   }
  }
    ...
    unsigned char *md, unsigned int *size, const EVP_MD *type, ENGINE *impl)
{
 ...
 EVP_MD_CTX_init(&ctx);
 EVP_MD_CTX_set_flags(&ctx, 0x0001);
...
EVP_MD_CTX_cleanup(EVP_MD_CTX *ctx)
{
 ...
 if (ctx->digest && ctx->digest->cleanup &&
     !EVP_MD_CTX_test_flags(ctx, 0x0002))
  ctx->digest->cleanup(ctx);
 if (ctx->digest && ctx->digest->ctx_size && ctx->md_data &&
     !EVP_MD_CTX_test_flags(ctx, 0x0004)) {
  explicit_bzero(ctx->md_data, ctx->digest->ctx_size);
...
EVP_MD_CTX_ctrl(EVP_MD_CTX *ctx, int type, int arg, void *ptr)
{
 ...
 if (!ctx->digest) {
  ERR_put_error(6,(195),(131),"evp/digest.c",415);
  return 0;
 }

 if (!ctx->digest->md_ctrl) {
  ERR_put_error(6,(195),(132),"evp/digest.c",420);
  return 0;
 }

 ret = ctx->digest->md_ctrl(ctx, type, arg, ptr);
 if (ret == -1) {
  ERR_put_error(6,(195),(133),"evp/digest.c",427)
                                           ;
  return 0;
 }
 return ret;
