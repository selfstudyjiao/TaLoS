    ...
    const unsigned char *from, int flen, const unsigned char *param, int plen)
{
 ...
 unsigned char *dbmask, seedmask[20];

 if (flen > emlen - 2 * 20 - 1) {
  ERR_put_error(4,(121),(110),"rsa/rsa_oaep.c",48)
                                        ;
  return 0;
 }

 if (emlen < 2 * 20 + 1) {
  ERR_put_error(4,(121),(120),"rsa/rsa_oaep.c",54)
                               ;
  return 0;
 }
 ...
 if (!EVP_Digest((void *)param, plen, db, 
# 62 "rsa/rsa_oaep.c" 3 4
                                         ((void *)0)
# 62 "rsa/rsa_oaep.c"
                                             , EVP_sha1(), 
# 62 "rsa/rsa_oaep.c" 3 4
...
# 62 "rsa/rsa_oaep.c"
 ...
 memset(db + 20, 0,
     emlen - flen - 2 * 20 - 1);
 db[emlen - flen - 20 - 1] = 0x01;
 memcpy(db + emlen - flen - 20, from, flen);
 arc4random_buf(seed, 20);

 dbmask = malloc(emlen - 20);
 if (dbmask == 
# 71 "rsa/rsa_oaep.c" 3 4
              ((void *)0)
# 71 "rsa/rsa_oaep.c"
                  ) {
  ERR_put_error(4,(121),((1|64)),"rsa/rsa_oaep.c",72);
  return 0;
 }

 if (MGF1(dbmask, emlen - 20, seed,
     20) < 0)
  return 0;
 for (i = 0; i < emlen - 20; i++)
  db[i] ^= dbmask[i];

 if (MGF1(seedmask, 20, db,
     emlen - 20) < 0)
  return 0;
 for (i = 0; i < 20; i++)
  seed[i] ^= seedmask[i];
    ...
    int plen)
{
 ...
 int lzero;
 unsigned char *db = 
# 100 "rsa/rsa_oaep.c" 3 4
                    ((void *)0)
# 100 "rsa/rsa_oaep.c"
                        ;
 unsigned char seed[20], phash[20];
 ...
 db = malloc(dblen + num);
 if (db == 
# 128 "rsa/rsa_oaep.c" 3 4
          ((void *)0)
# 128 "rsa/rsa_oaep.c"
              ) {
  ERR_put_error(4,(122),((1|64)),"rsa/rsa_oaep.c",130)
                           ;
  return -1;
 }
 ...
 maskeddb = padded_from + 20;

 if (MGF1(seed, 20, maskeddb, dblen))
  return -1;
 for (i = 0; i < 20; i++)
  seed[i] ^= padded_from[i];

 if (MGF1(db, dblen, seed, 20))
  return -1;
 for (i = 0; i < dblen; i++)
  db[i] ^= maskeddb[i];

 if (!EVP_Digest((void *)param, plen, phash, 
# 154 "rsa/rsa_oaep.c" 3 4
                                            ((void *)0)
# 154 "rsa/rsa_oaep.c"
                                                , EVP_sha1(), 
# 154 "rsa/rsa_oaep.c" 3 4
...
# 154 "rsa/rsa_oaep.c"
 ...
 if (timingsafe_memcmp(db, phash, 20) != 0 || bad)
  goto decoding_err;
 else {
  for (i = 20; i < dblen; i++)
   if (db[i] != 0x00)
  ...
  if (i == dblen || db[i] != 0x01)
   ...
   mlen = dblen - ++i;
   if (tlen < mlen) {
    ERR_put_error(4,(122),(109),"rsa/rsa_oaep.c",171)
                             ;
    mlen = -1;
   } else
    memcpy(to, db + i, mlen);
...
decoding_err:
 ...
 ERR_put_error(4,(122),(121),"rsa/rsa_oaep.c",185);
 free(db);
 return -1;
}
    ...
    long seedlen, const EVP_MD *dgst)
{
 ...
 for (i = 0; outlen < len; i++) {
  ...
  if (!EVP_DigestInit_ex(&c, dgst, 
# 210 "rsa/rsa_oaep.c" 3 4
                                  ((void *)0)
# 210 "rsa/rsa_oaep.c"
                                      ) ||
   ...
   goto err;
  if (outlen + mdlen <= len) {
   if (!EVP_DigestFinal_ex(&c, mask + outlen, 
# 215 "rsa/rsa_oaep.c" 3 4
                                             ((void *)0)
# 215 "rsa/rsa_oaep.c"
                                                 ))
    goto err;
   ...
   if (!EVP_DigestFinal_ex(&c, md, 
# 219 "rsa/rsa_oaep.c" 3 4
                                  ((void *)0)
# 219 "rsa/rsa_oaep.c"
                                      ))
    goto err;
