...
unsigned long
BN_mod_word(const BIGNUM *a, unsigned long w)
{
 ...
 for (i = a->top - 1; i >= 0; i--) {
  ...
  ret = ((ret << 32) | ((a->d[i] >> 32) &
      (0xffffffffL))) % w;
  ret = ((ret << 32) | (a->d[i] & (0xffffffffL))) % w;
...
unsigned long
BN_div_word(BIGNUM *a, unsigned long w)
{
 unsigned long ret = 0;
 int i, j;
...
int
BN_add_word(BIGNUM *a, unsigned long w)
{
 unsigned long l;
 int i;
 ...
 for (i = 0; w != 0 && i < a->top; i++) {
  a->d[i] = l = (a->d[i] + w) & (0xffffffffffffffffL);
  w = (w > l) ? 1 : 0;
 }
 if (w && i == a->top) {
  if ((((a->top + 1) <= (a)->dmax)?(a):bn_expand2((a),(a->top + 1))) == 
# 155 "bn/bn_word.c" 3 4
                                  ((void *)0)
# 155 "bn/bn_word.c"
                                      )
   return 0;
...
int
BN_sub_word(BIGNUM *a, unsigned long w)
{
 int i;
...
int
BN_mul_word(BIGNUM *a, unsigned long w)
{
 unsigned long ll;
