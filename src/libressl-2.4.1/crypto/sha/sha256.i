...
int SHA256_Final (unsigned char *md, SHA256_CTX *c)
{
 ...
 ({ unsigned int r=(c->Nh); asm ("bswapl %0":"=r"(r):"0"(r)); *((unsigned int *)(p))=r; (p)+=4; });
 ({ unsigned int r=(c->Nl); asm ("bswapl %0":"=r"(r):"0"(r)); *((unsigned int *)(p))=r; (p)+=4; });
 ...
 do { unsigned long ll; unsigned int nn; switch ((c)->md_len) { case 28: for (nn=0;nn<28/4;nn++) { ll=(c)->h[nn]; ({ unsigned int r=(ll); asm ("bswapl %0":"=r"(r):"0"(r)); *((unsigned int *)((md)))=r; ((md))+=4; }); } break; case 32: for (nn=0;nn<32/4;nn++) { ll=(c)->h[nn]; ({ unsigned int r=(ll); asm ("bswapl %0":"=r"(r):"0"(r)); *((unsigned int *)((md)))=r; ((md))+=4; }); } break; default: if ((c)->md_len > 32) return 0; for (nn=0;nn<(c)->md_len/4;nn++) { ll=(c)->h[nn]; ({ unsigned int r=(ll); asm ("bswapl %0":"=r"(r):"0"(r)); *((unsigned int *)((md)))=r; ((md))+=4; }); } break; } } while (0);
