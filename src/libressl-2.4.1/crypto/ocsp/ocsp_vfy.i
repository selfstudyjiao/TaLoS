...
# 62 "ocsp/ocsp_vfy.c" 2

static int ocsp_find_signer(X509 **psigner, OCSP_BASICRESP *bs,
    struct stack_st_X509 *certs, X509_STORE *st, unsigned long flags);
static X509 *ocsp_find_signer_sk(struct stack_st_X509 *certs, OCSP_RESPID *id);
static int ocsp_check_issuer(OCSP_BASICRESP *bs, struct stack_st_X509 *chain,
    unsigned long flags);
static int ocsp_check_ids(struct stack_st_OCSP_SINGLERESP *sresp, OCSP_CERTID **ret);
static int ocsp_match_issuerid(X509 *cert, OCSP_CERTID *cid,
    struct stack_st_OCSP_SINGLERESP *sresp);
static int ocsp_check_delegated(X509 *x, int flags);
static int ocsp_req_find_signer(X509 **psigner, OCSP_REQUEST *req,
    X509_NAME *nm, struct stack_st_X509 *certs, X509_STORE *st,
    unsigned long flags);


int
OCSP_basic_verify(OCSP_BASICRESP *bs, struct stack_st_X509 *certs, X509_STORE *st,
    unsigned long flags)
{
 X509 *signer, *x;
 struct stack_st_X509 *chain = 
# 82 "ocsp/ocsp_vfy.c" 3 4
                        ((void *)0)
# 82 "ocsp/ocsp_vfy.c"
                            ;
 X509_STORE_CTX ctx;
 ...
 if (!ret) {
  ERR_put_error(39,(105),(118),"ocsp/ocsp_vfy.c",89)
                                          ;
  goto end;
 }
 if ((ret == 2) && (flags & 0x200))
  flags |= 0x10;
 if (!(flags & 0x4)) {
  EVP_PKEY *skey;
  ...
  if (skey) {
   ret = ASN1_item_verify((&(OCSP_RESPDATA_it)), bs->signatureAlgorithm,bs->signature,bs->tbsResponseData,skey);
   EVP_PKEY_free(skey);
  }
  if (!skey || ret <= 0) {
   ERR_put_error(39,(105),(117),"ocsp/ocsp_vfy.c",104)
                                ;
   goto end;
  }
 }
...
# 112 "ocsp/ocsp_vfy.c"
  ...
  if (!init_res) {
   ret = -1;
   ERR_put_error(39,(105),(11),"ocsp/ocsp_vfy.c",118);
   goto end;
  }
  ...
  if (ret <= 0) {
   ...
   ERR_put_error(39,(105),(101),"ocsp/ocsp_vfy.c",129)
                                       ;
  ...
  x = ((X509 *)sk_value(((_STACK*) (1 ? (chain) : (struct stack_st_X509*)0)), (sk_num(((_STACK*) (1 ? (chain) : (struct stack_st_X509*)0))) - 1)));
  if (X509_check_trust(x, 180, 0) !=
   1) {
   ERR_put_error(39,(105),(112),"ocsp/ocsp_vfy.c",157)
                                  ;
   goto end;
  }
  ret = 1;
...
static int
ocsp_find_signer(X509 **psigner, OCSP_BASICRESP *bs, struct stack_st_X509 *certs,
    X509_STORE *st, unsigned long flags)
{
 X509 *signer;
...
static X509 *
ocsp_find_signer_sk(struct stack_st_X509 *certs, OCSP_RESPID *id)
{
 int i;
 unsigned char tmphash[20], *keyhash;
 X509 *x;
 ...
 if (id->value.byKey->length != 20)
  return 
# 206 "ocsp/ocsp_vfy.c" 3 4
        ((void *)0)
# 206 "ocsp/ocsp_vfy.c"
            ;
 ...
 for (i = 0; i < sk_num(((_STACK*) (1 ? (certs) : (struct stack_st_X509*)0))); i++) {
  ...
  X509_pubkey_digest(x, EVP_sha1(), tmphash, 
# 211 "ocsp/ocsp_vfy.c" 3 4
                                            ((void *)0)
# 211 "ocsp/ocsp_vfy.c"
                                                );
  if (!memcmp(keyhash, tmphash, 20))
   return x;
 }
 return 
# 215 "ocsp/ocsp_vfy.c" 3 4
       ((void *)0)
# 215 "ocsp/ocsp_vfy.c"
           ;
}
...
static int
ocsp_check_issuer(OCSP_BASICRESP *bs, struct stack_st_X509 *chain,
    unsigned long flags)
{
 struct stack_st_OCSP_SINGLERESP *sresp;
 X509 *signer, *sca;
 OCSP_CERTID *caid = 
# 224 "ocsp/ocsp_vfy.c" 3 4
                    ((void *)0)
# 224 "ocsp/ocsp_vfy.c"
                        ;
 int i;
 ...
 if (sk_num(((_STACK*) (1 ? (chain) : (struct stack_st_X509*)0))) <= 0) {
  ERR_put_error(39,(108),(105),"ocsp/ocsp_vfy.c",231)
                                      ;
  return -1;
 }
 ...
 signer = ((X509 *)sk_value(((_STACK*) (1 ? (chain) : (struct stack_st_X509*)0)), (0)));

 if (sk_num(((_STACK*) (1 ? (chain) : (struct stack_st_X509*)0))) > 1) {
  sca = ((X509 *)sk_value(((_STACK*) (1 ? (chain) : (struct stack_st_X509*)0)), (1)));
...
static int
ocsp_check_ids(struct stack_st_OCSP_SINGLERESP *sresp, OCSP_CERTID **ret)
{
 OCSP_CERTID *tmpid, *cid;
 int i, idcount;

 idcount = sk_num(((_STACK*) (1 ? (sresp) : (struct stack_st_OCSP_SINGLERESP*)0)));
 if (idcount <= 0) {
  ERR_put_error(39,(107),(111),"ocsp/ocsp_vfy.c",274)
                                                  ;
  return -1;
 }

 cid = ((OCSP_SINGLERESP *)sk_value(((_STACK*) (1 ? (sresp) : (struct stack_st_OCSP_SINGLERESP*)0)), (0)))->certId;

 *ret = 
# 280 "ocsp/ocsp_vfy.c" 3 4
       ((void *)0)
# 280 "ocsp/ocsp_vfy.c"
           ;

 for (i = 1; i < idcount; i++) {
  tmpid = ((OCSP_SINGLERESP *)sk_value(((_STACK*) (1 ? (sresp) : (struct stack_st_OCSP_SINGLERESP*)0)), (i)))->certId;
    ...
    struct stack_st_OCSP_SINGLERESP *sresp)
{
 ...
 if (cid) {
  ...
  if (!(dgst =
      EVP_get_digestbyname(OBJ_nid2sn(OBJ_obj2nid(cid->hashAlgorithm->algorithm))))) {
   ERR_put_error(39,(109),(119),"ocsp/ocsp_vfy.c",309)
                                     ;
   return -1;
  }
  ...
  if (!X509_NAME_digest(iname, dgst, md, 
# 320 "ocsp/ocsp_vfy.c" 3 4
                                        ((void *)0)
# 320 "ocsp/ocsp_vfy.c"
                                            ))
   return -1;
  if (memcmp(md, cid->issuerNameHash->data, mdlen))
   return 0;
  X509_pubkey_digest(cert, dgst, md, 
# 324 "ocsp/ocsp_vfy.c" 3 4
                                    ((void *)0)
# 324 "ocsp/ocsp_vfy.c"
                                        );
  ...
  for (i = 0; i < sk_num(((_STACK*) (1 ? (sresp) : (struct stack_st_OCSP_SINGLERESP*)0))); i++) {
   tmpid = ((OCSP_SINGLERESP *)sk_value(((_STACK*) (1 ? (sresp) : (struct stack_st_OCSP_SINGLERESP*)0)), (i)))->certId;
   ret = ocsp_match_issuerid(cert, tmpid, 
# 336 "ocsp/ocsp_vfy.c" 3 4
                                         ((void *)0)
# 336 "ocsp/ocsp_vfy.c"
                                             );
   if (ret <= 0)
...
ocsp_check_delegated(X509 *x, int flags)
{
 ...
 if ((x->ex_flags & 0x4) && (x->ex_xkusage & 0x20))
  return 1;
 ERR_put_error(39,(106),(103),"ocsp/ocsp_vfy.c",350);
 return 0;
}
...
int
OCSP_request_verify(OCSP_REQUEST *req, struct stack_st_X509 *certs, X509_STORE *store,
    unsigned long flags)
{
 X509 *signer;
 X509_NAME *nm;
 ...
 if (!req->optionalSignature) {
  ERR_put_error(39,(116),(128),"ocsp/ocsp_vfy.c",369);
  return 0;
 }
 gen = req->tbsRequest->requestorName;
 if (!gen || gen->type != 4) {
  ERR_put_error(39,(116),(129),"ocsp/ocsp_vfy.c",375)
                                            ;
  return 0;
 }
 ...
 if (ret <= 0) {
  ERR_put_error(39,(116),(118),"ocsp/ocsp_vfy.c",382)
                                          ;
  return 0;
 }
 if ((ret == 2) && (flags & 0x200))
 ...
 if (!(flags & 0x4)) {
  ...
  skey = X509_get_pubkey(signer);
  ret = ASN1_item_verify((&(OCSP_REQINFO_it)), req->optionalSignature->signatureAlgorithm, req->optionalSignature->signature,req->tbsRequest,skey);
  EVP_PKEY_free(skey);
  if (ret <= 0) {
   ERR_put_error(39,(116),(117),"ocsp/ocsp_vfy.c",395)
                                ;
   return 0;
  }
 }
...
# 404 "ocsp/ocsp_vfy.c"
  ...
  if (!init_res) {
   ERR_put_error(39,(116),(11),"ocsp/ocsp_vfy.c",409);
   return 0;
  }
  ...
  if (ret <= 0) {
   ...
   ERR_put_error(39,(116),(101),"ocsp/ocsp_vfy.c",420)
                                       ;
   ERR_asprintf_error_data("Verify error:%s",
