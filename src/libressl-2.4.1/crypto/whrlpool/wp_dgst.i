...
# 1 "<command-line>" 2
# 1 "whrlpool/wp_dgst.c"
# 55 "whrlpool/wp_dgst.c"
# 1 "whrlpool/wp_locl.h" 1
...
typedef int wchar_t;
# 4 "../include/openssl/whrlpool.h" 2
...
# 9 "../include/openssl/whrlpool.h" 2
...
# 4 "whrlpool/wp_locl.h" 2

void whirlpool_block(WHIRLPOOL_CTX *,const void *,size_t);
# 56 "whrlpool/wp_dgst.c" 2
...
# 157 "whrlpool/wp_dgst.c"
  while (bits)
   ...
   if (bitrem==inpgap)
    ...
    inpgap = 8-inpgap;
    bitoff += inpgap; bitrem = 0;
    bits -= inpgap; inpgap = 0;
    inp++;
    if (bitoff==512)
     {
     whirlpool_block(c,c->data,1);
...
int WHIRLPOOL_Final (unsigned char *md,WHIRLPOOL_CTX *c)
 ...
 if (byteoff > (512/8-(256/8)))
  {
  if (byteoff<512/8)
   memset(&c->data[byteoff],0,512/8-byteoff);
  whirlpool_block(c,c->data,1);
  byteoff = 0;
  }
 if (byteoff < (512/8-(256/8)))
...
unsigned char *WHIRLPOOL(const void *inp, size_t bytes,unsigned char *md)
 {
 WHIRLPOOL_CTX ctx;
 static unsigned char m[(512/8)];

 if (md == 
# 261 "whrlpool/wp_dgst.c" 3 4
          ((void *)0)
