...
static const unsigned long SQR_tb[16] =
 { 0, 1, 4, 5, 16, 17, 20, 21,
64, 65, 68, 69, 80, 81, 84, 85 };
# 315 "bn/bn_gf2m.c"
void bn_GF2m_mul_2x2(unsigned long *r, unsigned long a1, unsigned long a0, unsigned long b1,
    unsigned long b0);
...
BN_GF2m_add(BIGNUM *r, const BIGNUM *a, const BIGNUM *b)
{
 ...
 if ((((at->top) <= (r)->dmax)?(r):bn_expand2((r),(at->top))) == 
# 339 "bn/bn_gf2m.c" 3 4
                              ((void *)0)
# 339 "bn/bn_gf2m.c"
                                  )
  return 0;
...
BN_GF2m_mod_arr(BIGNUM *r, const BIGNUM *a, const int p[])
{
 ...
 if (a != r) {
  if (!(((a->top) <= (r)->dmax)?(r):bn_expand2((r),(a->top))))
   return 0;
  for (j = 0; j < a->top; j++) {
   r->d[j] = a->d[j];
  }
...
BN_GF2m_mod(BIGNUM *r, const BIGNUM *a, const BIGNUM *p)
{
 ...
 if (!ret || ret > (int)(sizeof(arr) / sizeof(arr[0]))) {
  ERR_put_error(3,(131),(106),"bn/bn_gf2m.c",474);
  return 0;
 }
    ...
    BN_CTX *ctx)
{
 ...
 BIGNUM *s;
 unsigned long x1, x0, y1, y0, zz[4];
...
# 502 "bn/bn_gf2m.c"
 ...
 zlen = a->top + b->top + 4;
 if (!(((zlen) <= (s)->dmax)?(s):bn_expand2((s),(zlen))))
  goto err;
 s->top = zlen;
 ...
 { unsigned long *ftl; int tmp_top = (s)->top; if (tmp_top > 0) { for (ftl= &((s)->d[tmp_top-1]); tmp_top > 0; tmp_top--) if (*(ftl--)) break; (s)->top = tmp_top; } ; };
...
# 553 "bn/bn_gf2m.c"
 ...
 if (!ret || ret > max) {
  ERR_put_error(3,(133),(106),"bn/bn_gf2m.c",557);
  goto err;
 }
...
BN_GF2m_mod_sqr_arr(BIGNUM *r, const BIGNUM *a, const int p[], BN_CTX *ctx)
{
 ...
 BN_CTX_start(ctx);
 if ((s = BN_CTX_get(ctx)) == 
# 578 "bn/bn_gf2m.c" 3 4
                             ((void *)0)
# 578 "bn/bn_gf2m.c"
                                 )
  goto err;
 if (!(((2 * a->top) <= (s)->dmax)?(s):bn_expand2((s),(2 * a->top))))
  goto err;

 for (i = a->top - 1; i >= 0; i--) {
  s->d[2 * i + 1] = SQR_tb[(a->d[i]) >> 60 & 0xF] << 56 | SQR_tb[(a->d[i]) >> 56 & 0xF] << 48 | SQR_tb[(a->d[i]) >> 52 & 0xF] << 40 | SQR_tb[(a->d[i]) >> 48 & 0xF] << 32 | SQR_tb[(a->d[i]) >> 44 & 0xF] << 24 | SQR_tb[(a->d[i]) >> 40 & 0xF] << 16 | SQR_tb[(a->d[i]) >> 36 & 0xF] << 8 | SQR_tb[(a->d[i]) >> 32 & 0xF];
  s->d[2 * i] = SQR_tb[(a->d[i]) >> 28 & 0xF] << 56 | SQR_tb[(a->d[i]) >> 24 & 0xF] << 48 | SQR_tb[(a->d[i]) >> 20 & 0xF] << 40 | SQR_tb[(a->d[i]) >> 16 & 0xF] << 32 | SQR_tb[(a->d[i]) >> 12 & 0xF] << 24 | SQR_tb[(a->d[i]) >> 8 & 0xF] << 16 | SQR_tb[(a->d[i]) >> 4 & 0xF] << 8 | SQR_tb[(a->d[i]) & 0xF];
 }
...
# 615 "bn/bn_gf2m.c"
 ...
 ret = BN_GF2m_poly2arr(p, arr, max);
 if (!ret || ret > max) {
  ERR_put_error(3,(136),(106),"bn/bn_gf2m.c",619);
  goto err;
 }
...
# 639 "bn/bn_gf2m.c"
 ...
 if ((b = BN_CTX_get(ctx)) == 
# 647 "bn/bn_gf2m.c" 3 4
                             ((void *)0)
# 647 "bn/bn_gf2m.c"
                                 )
  goto err;
 if ((c = BN_CTX_get(ctx)) == 
# 649 "bn/bn_gf2m.c" 3 4
                             ((void *)0)
# 649 "bn/bn_gf2m.c"
                                 )
  goto err;
 if ((u = BN_CTX_get(ctx)) == 
# 651 "bn/bn_gf2m.c" 3 4
                             ((void *)0)
# 651 "bn/bn_gf2m.c"
                                 )
  goto err;
 if ((v = BN_CTX_get(ctx)) == 
# 653 "bn/bn_gf2m.c" 3 4
                             ((void *)0)
# 653 "bn/bn_gf2m.c"
                                 )
  goto err;
...
# 699 "bn/bn_gf2m.c"
 {
  ...
  vbits = BN_num_bits(v),
  top = p->top;
  unsigned long *udp, *bdp, *vdp, *cdp;

  if (!(((top) <= (u)->dmax)?(u):bn_expand2((u),(top))))
                        goto err;
  udp = u->d;
  for (i = u->top; i < top; i++)
   udp[i] = 0;
  u->top = top;
  if (!(((top) <= (b)->dmax)?(b):bn_expand2((b),(top))))
                        goto err;
  bdp = b->d;
  bdp[0] = 1;
  for (i = 1; i < top; i++)
   bdp[i] = 0;
  b->top = top;
  if (!(((top) <= (c)->dmax)?(c):bn_expand2((c),(top))))
                        goto err;
  cdp = c->d;
  for (i = 0; i < top; i++)
   cdp[i] = 0;
  ...
  while (1) {
   while (ubits && !(udp[0]&1)) {
    unsigned long u0, u1, b0, b1, mask;

    u0 = udp[0];
    b0 = bdp[0];
    mask = (unsigned long)0 - (b0 & 1);
    b0 ^= p->d[0] & mask;
    for (i = 0; i < top - 1; i++) {
     u1 = udp[i + 1];
     udp[i] = ((u0 >> 1) |
         (u1 << (64 - 1))) & (0xffffffffffffffffL);
     u0 = u1;
     b1 = bdp[i + 1] ^ (p->d[i + 1] & mask);
     bdp[i] = ((b0 >> 1) |
         (b1 << (64 - 1))) & (0xffffffffffffffffL);
     b0 = b1;
    }
    udp[i] = u0 >> 1;
    bdp[i] = b0 >> 1;
   ...
   if (ubits == vbits) {
    ...
    while ((ul = udp[utop]) == 0 && utop)
     utop--;
    ubits = utop*64 + BN_num_bits_word(ul);
   }
  }
  { unsigned long *ftl; int tmp_top = (b)->top; if (tmp_top > 0) { for (ftl= &((b)->d[tmp_top-1]); tmp_top > 0; tmp_top--) if (*(ftl--)) break; (b)->top = tmp_top; } ; };
 }
...
BN_GF2m_mod_inv_arr(BIGNUM *r, const BIGNUM *xx, const int p[], BN_CTX *ctx)
{
 ...
 ;
 BN_CTX_start(ctx);
 if ((field = BN_CTX_get(ctx)) == 
# 820 "bn/bn_gf2m.c" 3 4
                                 ((void *)0)
# 820 "bn/bn_gf2m.c"
                                     )
  goto err;
...
# 842 "bn/bn_gf2m.c"
 ...
 BN_CTX_start(ctx);
 if ((xinv = BN_CTX_get(ctx)) == 
# 850 "bn/bn_gf2m.c" 3 4
                                ((void *)0)
# 850 "bn/bn_gf2m.c"
                                    )
  goto err;
    ...
    BN_CTX *ctx)
{
 ...
 if (BN_is_zero(b))
  return ((BN_set_word((r),1)));

 if (((((b)->top == 1) && ((b)->d[0] == (unsigned long)(1))) || (((1) == 0) && ((b)->top == 0))))
  return (BN_copy(r, a) != 
# 1006 "bn/bn_gf2m.c" 3 4
                          ((void *)0)
# 1006 "bn/bn_gf2m.c"
                              );
...
# 1052 "bn/bn_gf2m.c"
 ...
 ret = BN_GF2m_poly2arr(p, arr, max);
 if (!ret || ret > max) {
  ERR_put_error(3,(132),(106),"bn/bn_gf2m.c",1056);
  goto err;
 }
...
# 1114 "bn/bn_gf2m.c"
 ...
 ret = BN_GF2m_poly2arr(p, arr, max);
 if (!ret || ret > max) {
  ERR_put_error(3,(137),(106),"bn/bn_gf2m.c",1118);
  goto err;
 }
    ...
    BN_CTX *ctx)
{
 ...
 BN_CTX_start(ctx);
 if ((a = BN_CTX_get(ctx)) == 
# 1148 "bn/bn_gf2m.c" 3 4
                             ((void *)0)
# 1148 "bn/bn_gf2m.c"
                                 )
  goto err;
 if ((z = BN_CTX_get(ctx)) == 
# 1150 "bn/bn_gf2m.c" 3 4
                             ((void *)0)
# 1150 "bn/bn_gf2m.c"
                                 )
  goto err;
 if ((w = BN_CTX_get(ctx)) == 
# 1152 "bn/bn_gf2m.c" 3 4
                             ((void *)0)
# 1152 "bn/bn_gf2m.c"
                                 )
  goto err;
 ...
 {
  if ((rho = BN_CTX_get(ctx)) == 
# 1181 "bn/bn_gf2m.c" 3 4
                                ((void *)0)
# 1181 "bn/bn_gf2m.c"
                                    )
   goto err;
  if ((w2 = BN_CTX_get(ctx)) == 
# 1183 "bn/bn_gf2m.c" 3 4
                               ((void *)0)
# 1183 "bn/bn_gf2m.c"
                                   )
   goto err;
  if ((tmp = BN_CTX_get(ctx)) == 
# 1185 "bn/bn_gf2m.c" 3 4
                                ((void *)0)
# 1185 "bn/bn_gf2m.c"
                                    )
   goto err;
  do {
   ...
   count++;
  } while (BN_is_zero(w) && (count < 50));
  if (BN_is_zero(w)) {
   ERR_put_error(3,(135),(113),"bn/bn_gf2m.c",1211)
                                ;
   goto err;
  }
 }
 ...
 if (!BN_GF2m_add(w, z, w))
  goto err;
 if (BN_ucmp((w), (a))) {
  ERR_put_error(3,(135),(116),"bn/bn_gf2m.c",1221);
  goto err;
 }
...
# 1251 "bn/bn_gf2m.c"
 ...
 ret = BN_GF2m_poly2arr(p, arr, max);
 if (!ret || ret > max) {
  ERR_put_error(3,(134),(106),"bn/bn_gf2m.c",1255);
  goto err;
 }
