...
EVP_PKEY_sign_init(EVP_PKEY_CTX *ctx)
{
 ...
 if (!ctx || !ctx->pmeth || !ctx->pmeth->sign) {
  ERR_put_error(6,(141),(150),"evp/pmeth_fn.c",91)
                                                     ;
  return -2;
 }
 ctx->operation = (1<<3);
...
{
 if (!ctx || !ctx->pmeth || !ctx->pmeth->sign) {
  ERR_put_error(6,(140),(150),"evp/pmeth_fn.c",109)
                                                     ;
  return -2;
 }
 if (ctx->operation != (1<<3)) {
  ERR_put_error(6,(140),(151),"evp/pmeth_fn.c",113);
  return -1;
 }
 if (ctx->pmeth->flags & 2) { size_t pksize = (size_t)EVP_PKEY_size(ctx->pkey); if (!sig) { *siglen = pksize; return 1; } else if (*siglen < pksize) { ERR_put_error(6,(140),(155),"evp/pmeth_fn.c",116); return 0; } }
...
EVP_PKEY_verify_init(EVP_PKEY_CTX *ctx)
{
 ...
 if (!ctx || !ctx->pmeth || !ctx->pmeth->verify) {
  ERR_put_error(6,(143),(150),"evp/pmeth_fn.c",127)
                                                     ;
  return -2;
 }
 ctx->operation = (1<<4);
...
{
 if (!ctx || !ctx->pmeth || !ctx->pmeth->verify) {
  ERR_put_error(6,(142),(150),"evp/pmeth_fn.c",145)
                                                     ;
  return -2;
 }
 if (ctx->operation != (1<<4)) {
  ERR_put_error(6,(142),(151),"evp/pmeth_fn.c",149);
  return -1;
 }
...
EVP_PKEY_verify_recover_init(EVP_PKEY_CTX *ctx)
{
 ...
 if (!ctx || !ctx->pmeth || !ctx->pmeth->verify_recover) {
  ERR_put_error(6,(145),(150),"evp/pmeth_fn.c",162)
                                                     ;
  return -2;
 }
 ctx->operation = (1<<5);
...
{
 if (!ctx || !ctx->pmeth || !ctx->pmeth->verify_recover) {
  ERR_put_error(6,(144),(150),"evp/pmeth_fn.c",180)
                                                     ;
  return -2;
 }
 if (ctx->operation != (1<<5)) {
  ERR_put_error(6,(144),(151),"evp/pmeth_fn.c",185)
                                     ;
  return -1;
 }
 if (ctx->pmeth->flags & 2) { size_t pksize = (size_t)EVP_PKEY_size(ctx->pkey); if (!rout) { *routlen = pksize; return 1; } else if (*routlen < pksize) { ERR_put_error(6,(144),(155),"evp/pmeth_fn.c",188); return 0; } }
...
EVP_PKEY_encrypt_init(EVP_PKEY_CTX *ctx)
{
 ...
 if (!ctx || !ctx->pmeth || !ctx->pmeth->encrypt) {
  ERR_put_error(6,(139),(150),"evp/pmeth_fn.c",199)
                                                     ;
  return -2;
 }
 ctx->operation = (1<<8);
...
{
 if (!ctx || !ctx->pmeth || !ctx->pmeth->encrypt) {
  ERR_put_error(6,(105),(150),"evp/pmeth_fn.c",217)
                                                     ;
  return -2;
 }
 if (ctx->operation != (1<<8)) {
  ERR_put_error(6,(105),(151),"evp/pmeth_fn.c",221);
  return -1;
 }
 if (ctx->pmeth->flags & 2) { size_t pksize = (size_t)EVP_PKEY_size(ctx->pkey); if (!out) { *outlen = pksize; return 1; } else if (*outlen < pksize) { ERR_put_error(6,(105),(155),"evp/pmeth_fn.c",224); return 0; } }
...
EVP_PKEY_decrypt_init(EVP_PKEY_CTX *ctx)
{
 ...
 if (!ctx || !ctx->pmeth || !ctx->pmeth->decrypt) {
  ERR_put_error(6,(138),(150),"evp/pmeth_fn.c",235)
                                                     ;
  return -2;
 }
 ctx->operation = (1<<9);
...
{
 if (!ctx || !ctx->pmeth || !ctx->pmeth->decrypt) {
  ERR_put_error(6,(104),(150),"evp/pmeth_fn.c",253)
                                                     ;
  return -2;
 }
 if (ctx->operation != (1<<9)) {
  ERR_put_error(6,(104),(151),"evp/pmeth_fn.c",257);
  return -1;
 }
 if (ctx->pmeth->flags & 2) { size_t pksize = (size_t)EVP_PKEY_size(ctx->pkey); if (!out) { *outlen = pksize; return 1; } else if (*outlen < pksize) { ERR_put_error(6,(104),(155),"evp/pmeth_fn.c",260); return 0; } }
...
EVP_PKEY_derive_init(EVP_PKEY_CTX *ctx)
{
 ...
 if (!ctx || !ctx->pmeth || !ctx->pmeth->derive) {
  ERR_put_error(6,(154),(150),"evp/pmeth_fn.c",271)
                                                     ;
  return -2;
 }
 ctx->operation = (1<<10);
...
EVP_PKEY_derive_set_peer(EVP_PKEY_CTX *ctx, EVP_PKEY *peer)
{
 ...
 if (!ctx || !ctx->pmeth || !(ctx->pmeth->derive ||
     ...
     !ctx->pmeth->ctrl) {
  ERR_put_error(6,(155),(150),"evp/pmeth_fn.c",292)
                                                     ;
  return -2;
 }
 if (ctx->operation != (1<<10) &&
     ctx->operation != (1<<8) &&
     ctx->operation != (1<<9)) {
  ERR_put_error(6,(155),(151),"evp/pmeth_fn.c",299)
                                     ;
  return -1;
 }

 ret = ctx->pmeth->ctrl(ctx, 2, 0, peer);
 ...
 if (!ctx->pkey) {
  ERR_put_error(6,(155),(154),"evp/pmeth_fn.c",312);
  return -1;
 }

 if (ctx->pkey->type != peer->type) {
  ERR_put_error(6,(155),(101),"evp/pmeth_fn.c",318)
                                ;
  return -1;
 }
 ...
 if (!EVP_PKEY_missing_parameters(peer) &&
  ...
  ERR_put_error(6,(155),(153),"evp/pmeth_fn.c",330)
                                 ;
  return -1;
 }
 ...
 ret = ctx->pmeth->ctrl(ctx, 2, 1, peer);

 if (ret <= 0) {
  ctx->peerkey = 
# 340 "evp/pmeth_fn.c" 3 4
                ((void *)0)
# 340 "evp/pmeth_fn.c"
                    ;
  return ret;
 ...
 CRYPTO_add_lock(&peer->references,1,10,"evp/pmeth_fn.c",344);
 return 1;
}
...
{
 if (!ctx || !ctx->pmeth || !ctx->pmeth->derive) {
  ERR_put_error(6,(153),(150),"evp/pmeth_fn.c",353)
                                                     ;
  return -2;
 }
 if (ctx->operation != (1<<10)) {
  ERR_put_error(6,(153),(151),"evp/pmeth_fn.c",357);
  return -1;
 }
 if (ctx->pmeth->flags & 2) { size_t pksize = (size_t)EVP_PKEY_size(ctx->pkey); if (!key) { *pkeylen = pksize; return 1; } else if (*pkeylen < pksize) { ERR_put_error(6,(153),(155),"evp/pmeth_fn.c",360); return 0; } }
