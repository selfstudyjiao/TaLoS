...
X509_CERT_AUX_print(BIO *out, X509_CERT_AUX *aux, int indent)
{
 ...
 if (aux->trust) {
  ...
  BIO_printf(out, "%*sTrusted Uses:\n%*s",
      indent, "", indent + 2, "");
  for (i = 0; i < sk_num(((_STACK*) (1 ? (aux->trust) : (struct stack_st_ASN1_OBJECT*)0))); i++) {
   if (!first)
    BIO_puts(out, ", ");
   else
    first = 0;
   OBJ_obj2txt(oidstr, sizeof oidstr,
       ((ASN1_OBJECT *)sk_value(((_STACK*) (1 ? (aux->trust) : (struct stack_st_ASN1_OBJECT*)0)), (i))), 0);
   BIO_puts(out, oidstr);
  }
  BIO_puts(out, "\n");
 } else
 ...
 if (aux->reject) {
  ...
  BIO_printf(out, "%*sRejected Uses:\n%*s",
      indent, "", indent + 2, "");
  for (i = 0; i < sk_num(((_STACK*) (1 ? (aux->reject) : (struct stack_st_ASN1_OBJECT*)0))); i++) {
   if (!first)
    BIO_puts(out, ", ");
   else
    first = 0;
   OBJ_obj2txt(oidstr, sizeof oidstr,
       ((ASN1_OBJECT *)sk_value(((_STACK*) (1 ? (aux->reject) : (struct stack_st_ASN1_OBJECT*)0)), (i))), 0);
   BIO_puts(out, oidstr);
  }
  BIO_puts(out, "\n");
 } else
