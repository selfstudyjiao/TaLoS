...
# 62 "evp/e_rc4_hmac_md5.c" 2
# 72 "evp/e_rc4_hmac_md5.c"
typedef struct {
 RC4_KEY ks;
...
rc4_hmac_md5_init_key(EVP_CIPHER_CTX *ctx, const unsigned char *inkey,
    const unsigned char *iv, int enc)
{
 EVP_RC4_HMAC_MD5 *key = ((EVP_RC4_HMAC_MD5 *)(ctx)->cipher_data);
    ...
    const unsigned char *in, size_t len)
{
 ...
 if (plen != ((size_t)-1) && len != (plen + 16))
  return 0;

 if (ctx->encrypt) {
  if (plen == ((size_t)-1))
   plen = len;
# 160 "evp/e_rc4_hmac_md5.c"
  MD5_Update(&key->md, in + 0, plen - 0);
  ...
  if (plen!=len) {
   ...
   key->md = key->tail;
   MD5_Update(&key->md, out + plen, 16);
   MD5_Final(out + plen, &key->md);


   RC4(&key->ks, len - 0, out + 0,
       out + 0);
  } else {
   RC4(&key->ks, len - 0, in + 0,
       out + 0);
  }
 } else {
  unsigned char mac[16];
# 210 "evp/e_rc4_hmac_md5.c"
  RC4(&key->ks, len - 0, in + 0, out + 0);
  if (plen!=((size_t)-1)) {
   MD5_Update(&key->md, out + 0, plen - 0);


   MD5_Final(mac, &key->md);
   key->md = key->tail;
   MD5_Update(&key->md, mac, 16);
   MD5_Final(mac, &key->md);

   if (memcmp(out + plen, mac, 16))
    return 0;
  } else {
   MD5_Update(&key->md, out + 0, len - 0);
  }
 }
...
# 296 "evp/e_rc4_hmac_md5.c" 3 4
((void *)0)
# 296 "evp/e_rc4_hmac_md5.c"
    ,
 sizeof(EVP_RC4_HMAC_MD5),
 
# 298 "evp/e_rc4_hmac_md5.c" 3 4
((void *)0)
# 298 "evp/e_rc4_hmac_md5.c"
    ,
 
# 299 "evp/e_rc4_hmac_md5.c" 3 4
((void *)0)
# 299 "evp/e_rc4_hmac_md5.c"
    ,
 rc4_hmac_md5_ctrl,
 
# 301 "evp/e_rc4_hmac_md5.c" 3 4
((void *)0)

# 302 "evp/e_rc4_hmac_md5.c"
};
