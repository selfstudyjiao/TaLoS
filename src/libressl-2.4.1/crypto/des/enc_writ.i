...
# 92 "des/enc_writ.c"
 ...
 if (outbuf == 
# 98 "des/enc_writ.c" 3 4
              ((void *)0)
# 98 "des/enc_writ.c"
                  )
  {
  outbuf=malloc(((1024*16)+4)+4);
  if (outbuf == 
# 101 "des/enc_writ.c" 3 4
               ((void *)0)
# 101 "des/enc_writ.c"
                   ) return(-1);
  }
 ...
 if (len > (1024*16))
  ...
  for (i=0; i<len; i+=k)
   ...
   k=DES_enc_write(fd,&(buf[i]),
    ((len-i) > (1024*16))?(1024*16):(len-i),sched,iv);
   if (k < 0)
    return(k);
   else
    j+=k;
 ...
 p=outbuf;
 (*((p)++)=(unsigned char)(((len)>>24L)&0xff), *((p)++)=(unsigned char)(((len)>>16L)&0xff), *((p)++)=(unsigned char)(((len)>> 8L)&0xff), *((p)++)=(unsigned char)(((len) )&0xff));
 ...
 if (DES_rw_mode & 1)
  DES_pcbc_encrypt(cp,&(outbuf[4]),(len<8)?8:len,sched,iv,
     1);
 else
  DES_cbc_encrypt(cp,&(outbuf[4]),(len<8)?8:len,sched,iv,
    1);
