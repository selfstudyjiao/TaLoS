    ...
    unsigned char *mac, unsigned int *maclen)
{
 const EVP_MD *md_type;
 HMAC_CTX hmac;
 unsigned char key[64], *salt;
 int saltlen, iter;
 int md_size;

 if (!(OBJ_obj2nid((p12->authsafes)->type) == 21)) {
  ERR_put_error(35,(107),(121),"pkcs12/p12_mutl.c",84)
                                     ;
  return 0;
 }
 ...
 if (!(md_type = EVP_get_digestbyname(OBJ_nid2sn(OBJ_obj2nid(p12->mac->dinfo->algor->algorithm)))
                                       )) {
  ERR_put_error(35,(107),(118),"pkcs12/p12_mutl.c",97)
                                        ;
  return 0;
 }
 ...
 if (md_size < 0)
  return 0;
 if (!PKCS12_key_gen_asc(pass, passlen, salt, saltlen, 3, iter,
     md_size, key, md_type)) {
  ERR_put_error(35,(107),(107),"pkcs12/p12_mutl.c",105);
  return 0;
 }
 HMAC_CTX_init(&hmac);
 if (!HMAC_Init_ex(&hmac, key, md_size, md_type, 
# 109 "pkcs12/p12_mutl.c" 3 4
                                                ((void *)0)
# 109 "pkcs12/p12_mutl.c"
                                                    ) ||
...
PKCS12_verify_mac(PKCS12 *p12, const char *pass, int passlen)
{
 ...
 if (p12->mac == 
# 127 "pkcs12/p12_mutl.c" 3 4
                ((void *)0)
# 127 "pkcs12/p12_mutl.c"
                    ) {
  ERR_put_error(35,(126),(108),"pkcs12/p12_mutl.c",128);
  return 0;
 }
 if (!PKCS12_gen_mac(p12, pass, passlen, mac, &maclen)) {
  ERR_put_error(35,(126),(109),"pkcs12/p12_mutl.c",133)
                                    ;
  return 0;
 }
    ...
    int saltlen, int iter, const EVP_MD *md_type)
{
 ...
 if (PKCS12_setup_mac(p12, iter, salt, saltlen, md_type) ==
     0) {
  ERR_put_error(35,(123),(110),"pkcs12/p12_mutl.c",155);
  return 0;
 }
 if (!PKCS12_gen_mac(p12, pass, passlen, mac, &maclen)) {
  ERR_put_error(35,(123),(109),"pkcs12/p12_mutl.c",160)
                                    ;
  return 0;
 }
 if (!(ASN1_STRING_set(p12->mac->dinfo->digest, mac, maclen))) {
  ERR_put_error(35,(123),(111),"pkcs12/p12_mutl.c",165)
                                    ;
  return 0;
 }
 return 1;
    ...
    const EVP_MD *md_type)
{
 ...
 if (iter > 1) {
  if (!(p12->mac->iter = ASN1_INTEGER_new())) {
   ERR_put_error(35,(122),((1|64)),"pkcs12/p12_mutl.c",181)
                            ;
   return 0;
  }
  if (!ASN1_INTEGER_set(p12->mac->iter, iter)) {
   ERR_put_error(35,(122),((1|64)),"pkcs12/p12_mutl.c",186)
                            ;
   return 0;
  }
 }
 ...
 if (!(p12->mac->salt->data = malloc(saltlen))) {
  ERR_put_error(35,(122),((1|64)),"pkcs12/p12_mutl.c",193);
  return 0;
 }
 ...
 if (!(p12->mac->dinfo->algor->parameter = ASN1_TYPE_new())) {
  ERR_put_error(35,(122),((1|64)),"pkcs12/p12_mutl.c",203);
  return 0;
 }
