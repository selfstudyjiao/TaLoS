...
typedef struct {
 unsigned long data[(16 / sizeof(unsigned long))];
} aes_block_t;
# 79 "aes/aes_ige.c"
void
    ...
    const AES_KEY *key, unsigned char *ivec, const int enc)
{
 ...
 (void)(((length % 16) == 0) ? 0 : (OpenSSLDie("aes/aes_ige.c", 86, "(length % AES_BLOCK_SIZE) == 0"),1));
 ...
 if (1 == enc) {
  if (in != out && (1 ||
   ...
   aes_block_t *iv2p = (aes_block_t *)(ivec + 16);
   ...
   memcpy(ivec, ivp->data, 16);
   memcpy(ivec + 16, iv2p->data, 16);
  } else {
   aes_block_t tmp, tmp2;
   aes_block_t iv;
   aes_block_t iv2;

   (iv) = *(const aes_block_t *)(ivec);
   (iv2) = *(const aes_block_t *)(ivec + 16);

   while (len) {
    (tmp) = *(const aes_block_t *)(in);
    for (n = 0; n < (16 / sizeof(unsigned long)); ++n)
   ...
   memcpy(ivec, iv.data, 16);
   memcpy(ivec + 16, iv2.data, 16);
  }
 } else {
  if (in != out && (1 ||
      ((size_t)in|(size_t)out|(size_t)ivec) %
