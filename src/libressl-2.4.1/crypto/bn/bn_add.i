...
{
 int max, min, dif;
 unsigned long *ap, *bp, *rp, carry, t1, t2;
 const BIGNUM *tmp;
 ...
 if ((((max + 1) <= (r)->dmax)?(r):bn_expand2((r),(max + 1))) == 
# 128 "bn/bn_add.c" 3 4
                              ((void *)0)
# 128 "bn/bn_add.c"
                                  )
...
BN_usub(BIGNUM *r, const BIGNUM *a, const BIGNUM *b)
{
 ...
 if (dif < 0)
 {
  ERR_put_error(3,(115),(100),"bn/bn_add.c",185);
  return (0);
 }
...
# 189 "bn/bn_add.c"
 ...
 for (i = min; i != 0; i--) {
  ...
  if (carry) {
   carry = (t1 <= t2);
   t1 = (t1 - t2 - 1)&(0xffffffffffffffffL);
  } else {
   carry = (t1 < t2);
   t1 = (t1 - t2)&(0xffffffffffffffffL);
  }
  *(rp++) = t1&(0xffffffffffffffffL);
 }
