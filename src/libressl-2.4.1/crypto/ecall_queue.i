...
struct mpmcq* newmpmc(int nslots) {
 if (nslots <= 0) return 
# 32 "ecall_queue.c" 3 4
                        ((void *)0)
# 32 "ecall_queue.c"
                            ;
# 122 "ecall_queue.c"
 return 
# 122 "ecall_queue.c" 3 4
       ((void *)0)
# 122 "ecall_queue.c"
           ;
...
int mpmc_wait_for_enqueue(struct mpmcq *q, int tid, int ncycles) {
 int status = __atomic_load_n(&q->buffer[tid].status, 0);
 int i = 0;
 while (status != 0 && i<ncycles) {
  mpmc_pause();
  status = __atomic_load_n(&q->buffer[tid].status, 0);
  i++;
 }
...
void mpmc_enqueue(struct mpmcq *q, enum transition_type type, int tid, size_t len) {
 ...
 __atomic_store_n(&q->buffer[tid].status, 1, 0);
}

int mpmc_slot_taken(struct mpmcq *q, int tid) {
 return __atomic_load_n(&q->buffer[tid].status, 0) == 1;
}
...
enum transition_type mpmc_dequeue(struct mpmcq *q, int tid, void **data) {
 ...
 int expected = 1;
 int desired = 0;
 if (__atomic_compare_exchange_n(&q->buffer[tid].status, &expected, desired, 0, 0, 0)) {
...
void mpmc_enqueue_result(struct mpmcq *q, int tid, size_t len) {
 __atomic_store_n(&q->buffer[tid].status, 2, 0);
}
...
void mpmc_dequeue_result(struct mpmcq *q, int tid, void **data) {
 ...
 __atomic_store_n(&q->buffer[tid].status, 0, 0);
}
