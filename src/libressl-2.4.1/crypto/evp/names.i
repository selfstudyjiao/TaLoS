...
# 70 "evp/names.c"
 ...
 r = OBJ_NAME_add(OBJ_nid2sn(c->nid), 0x02,
     (const char *)c);
 if (r == 0)
  return (0);
 check_defer(c->nid);
 r = OBJ_NAME_add(OBJ_nid2ln(c->nid), 0x02,
     (const char *)c);
 return (r);
}
...
EVP_add_digest(const EVP_MD *md)
{
 ...
 r = OBJ_NAME_add(name, 0x01, (const char *)md);
 if (r == 0)
  return (0);
 check_defer(md->type);
 r = OBJ_NAME_add(OBJ_nid2ln(md->type), 0x01,
     (const char *)md);
 if (r == 0)
  return (0);
...
EVP_get_cipherbyname(const char *name)
{
 ...
 cp = (const EVP_CIPHER *)OBJ_NAME_get(name, 0x02);
 return (cp);
}
...
EVP_cleanup(void)
{
 OBJ_NAME_cleanup(0x02);
 OBJ_NAME_cleanup(0x01);
...
# 172 "evp/names.c" 3 4
        ((void *)0)
# 172 "evp/names.c"
            , nm->name, nm->data, dc->arg);
 else
  dc->fn((const EVP_CIPHER *)nm->data, nm->name, 
# 174 "evp/names.c" 3 4
                                                ((void *)0)
# 174 "evp/names.c"
                                                    , dc->arg);
    ...
    const char *to, void *x), void *arg)
{
 ...
 dc.fn = fn;
 dc.arg = arg;
 OBJ_NAME_do_all(0x02, do_all_cipher_fn, &dc);
}
...
# 212 "evp/names.c"
  ...
  dc->fn((const EVP_MD *)nm->data, nm->name, 
# 214 "evp/names.c" 3 4
                                            ((void *)0)
# 214 "evp/names.c"
                                                , dc->arg);
    ...
    void *x), void *arg)
{
 ...
 dc.fn = fn;
 dc.arg = arg;
 OBJ_NAME_do_all(0x01, do_all_md_fn, &dc);
}
    ...
    const char *from, const char *to, void *x), void *arg)
{
 ...
 dc.fn = fn;
 dc.arg = arg;
 OBJ_NAME_do_all_sorted(0x01, do_all_md_fn, &dc);
}
