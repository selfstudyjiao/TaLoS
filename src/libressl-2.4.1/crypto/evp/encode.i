 ...
 *outl = 0;
 if (inl == 0)
  return;
 (void)((ctx->length <= (int)sizeof(ctx->enc_data)) ? 0 : (OpenSSLDie("evp/encode.c", 133, "ctx->length <= (int)sizeof(ctx->enc_data)"),1));
 ...
 while (inl >= ctx->length && total <= 0x7fffffff) {
...
EVP_EncodeBlock(unsigned char *t, const unsigned char *f, int dlen)
{
 ...
 for (i = dlen; i > 0; i -= 3) {
   ...
   *(t++) = (data_bin2ascii[(l >> 18L)&0x3f]);
   *(t++) = (data_bin2ascii[(l >> 12L)&0x3f]);
   *(t++) = (data_bin2ascii[(l >> 6L)&0x3f]);
   *(t++) = (data_bin2ascii[(l)&0x3f]);
  } else {
   l = ((unsigned long)f[0]) << 16L;
   if (i == 2)
   ...
   *(t++) = (data_bin2ascii[(l >> 18L)&0x3f]);
   *(t++) = (data_bin2ascii[(l >> 12L)&0x3f]);
   *(t++) = (i == 1) ? '=' : (data_bin2ascii[(l >> 6L)&0x3f]);
   *(t++) = '=';
  }
  ret += 4;
  f += 3;
    ...
    const unsigned char *in, int inl)
{
 ...
 if ((inl == 0) || ((n == 0) && ((data_ascii2bin[(in[0])&0x7f]) == 0xF2))) {
  rv = 0;
  goto end;
 }
 ...
 for (i = 0; i < inl; i++) {
  ...
  tmp= *(in++);
  v = (data_ascii2bin[(tmp)&0x7f]);

  if (!(((v)|0x13) == 0xF3)) {
   (void)((n < (int)sizeof(ctx->enc_data)) ? 0 : (OpenSSLDie("evp/encode.c", 261, "n < (int)sizeof(ctx->enc_data)"),1));
   d[n++] = tmp;
   ln++;
  } else if (v == 0xFF) {
   rv = -1;
...
EVP_DecodeBlock(unsigned char *t, const unsigned char *f, int n)
{
 ...
 for (i = 0; i < n; i += 4) {
  ...
  b = (data_ascii2bin[(*(f++))&0x7f]);
  c = (data_ascii2bin[(*(f++))&0x7f]);
  d = (data_ascii2bin[(*(f++))&0x7f]);
  if ((a & 0x80) || (b & 0x80) ||
      (c & 0x80) || (d & 0x80))
   return (-1);
