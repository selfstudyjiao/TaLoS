...
# 63 "pkcs12/p12_kiss.c" 2



static int parse_pk12( PKCS12 *p12, const char *pass, int passlen,
    EVP_PKEY **pkey, struct stack_st_X509 *ocerts);

static int parse_bags( struct stack_st_PKCS12_SAFEBAG *bags, const char *pass,
    int passlen, EVP_PKEY **pkey, struct stack_st_X509 *ocerts);
...
PKCS12_parse(PKCS12 *p12, const char *pass, EVP_PKEY **pkey, X509 **cert,
    struct stack_st_X509 **ca)
{
 struct stack_st_X509 *ocerts = 
# 85 "pkcs12/p12_kiss.c" 3 4
                         ((void *)0)
# 85 "pkcs12/p12_kiss.c"
                             ;
 X509 *x = 
# 86 "pkcs12/p12_kiss.c" 3 4
          ((void *)0)
# 86 "pkcs12/p12_kiss.c"
              ;


 if (!p12) {
  ERR_put_error(35,(118),(105),"pkcs12/p12_kiss.c",91)
                                           ;
  return 0;
 }
 ...
 if (pkey)
  *pkey = 
# 96 "pkcs12/p12_kiss.c" 3 4
         ((void *)0)
# 96 "pkcs12/p12_kiss.c"
             ;
 if (cert)
  *cert = 
# 98 "pkcs12/p12_kiss.c" 3 4
         ((void *)0)
# 98 "pkcs12/p12_kiss.c"
             ;
# 108 "pkcs12/p12_kiss.c"
 if (!pass || !*pass) {
  if (PKCS12_verify_mac(p12, 
# 109 "pkcs12/p12_kiss.c" 3 4
                            ((void *)0)
# 109 "pkcs12/p12_kiss.c"
                                , 0))
   pass = 
# 110 "pkcs12/p12_kiss.c" 3 4
         ((void *)0)
# 110 "pkcs12/p12_kiss.c"
             ;
  else if (PKCS12_verify_mac(p12, "", 0))
   pass = "";
  else {
   ERR_put_error(35,(118),(113),"pkcs12/p12_kiss.c",115)
                                   ;
   goto err;
  }
 } else if (!PKCS12_verify_mac(p12, pass, -1)) {
  ERR_put_error(35,(118),(113),"pkcs12/p12_kiss.c",119);
  goto err;
 }


 ocerts = ((struct stack_st_X509 *)sk_new_null());
 if (!ocerts) {
  ERR_put_error(35,(118),((1|64)),"pkcs12/p12_kiss.c",126);
  return 0;
 }

 if (!parse_pk12 (p12, pass, -1, pkey, ocerts)) {
  ERR_put_error(35,(118),(114),"pkcs12/p12_kiss.c",131);
  goto err;
 }

 while ((x = (X509 *)sk_pop(((_STACK*) (1 ? (ocerts) : (struct stack_st_X509*)0))))) {
  if (pkey && *pkey && cert && !*cert) {
   if (X509_check_private_key(x, *pkey)) {
    *cert = x;
    x = 
# 139 "pkcs12/p12_kiss.c" 3 4
       ((void *)0)
# 139 "pkcs12/p12_kiss.c"
           ;
   }
  }
  ...
  if (ca && x) {
   ...
   if (!*ca)
    goto err;
   if (!sk_push(((_STACK*) (1 ? (*ca) : (struct stack_st_X509*)0)), ((void*) (1 ? (x) : (X509*)0))))
    goto err;
   x = 
# 150 "pkcs12/p12_kiss.c" 3 4
      ((void *)0)
# 150 "pkcs12/p12_kiss.c"
          ;
  }
  X509_free(x);
 }
 ...
 if (ocerts)
  sk_pop_free(((_STACK*) (1 ? (ocerts) : (struct stack_st_X509*)0)), ((void (*)(void *)) ((1 ? (X509_free) : (void (*)(X509 *))0))));
    ...
    struct stack_st_X509 *ocerts)
{
 ...
 if (!(asafes = PKCS12_unpack_authsafes (p12)))
  return 0;
 for (i = 0; i < sk_num(((_STACK*) (1 ? (asafes) : (struct stack_st_PKCS7*)0))); i++) {
  p7 = ((PKCS7 *)sk_value(((_STACK*) (1 ? (asafes) : (struct stack_st_PKCS7*)0)), (i)));
  ...
  if (bagnid == 21) {
   ...
   continue;
  if (!bags) {
   sk_pop_free(((_STACK*) (1 ? (asafes) : (struct stack_st_PKCS7*)0)), ((void (*)(void *)) ((1 ? (PKCS7_free) : (void (*)(PKCS7 *))0))));
   return 0;
  }
...
static int
parse_bags(struct stack_st_PKCS12_SAFEBAG *bags, const char *pass, int passlen,
    EVP_PKEY **pkey, struct stack_st_X509 *ocerts)
{
 int i;

 for (i = 0; i < sk_num(((_STACK*) (1 ? (bags) : (struct stack_st_PKCS12_SAFEBAG*)0))); i++) {
  if (!parse_bag(((PKCS12_SAFEBAG *)sk_value(((_STACK*) (1 ? (bags) : (struct stack_st_PKCS12_SAFEBAG*)0)), (i))), pass, passlen,
      pkey, ocerts))
   return 0;
 }
 return 1;
    ...
    struct stack_st_X509 *ocerts)
{
 ...
 X509 *x509;
 ASN1_TYPE *attrib;
 ASN1_BMPSTRING *fname = 
# 229 "pkcs12/p12_kiss.c" 3 4
                        ((void *)0)
# 229 "pkcs12/p12_kiss.c"
                            ;
 ASN1_OCTET_STRING *lkid = 
# 230 "pkcs12/p12_kiss.c" 3 4
                          ((void *)0)
# 230 "pkcs12/p12_kiss.c"
                              ;

 if ((attrib = PKCS12_get_attr_gen(bag->attrib, 156)))
  fname = attrib->value.bmpstring;

 if ((attrib = PKCS12_get_attr_gen(bag->attrib, 157)))
 ...
 case 152:
  if (OBJ_obj2nid((bag)->value.bag->type) != 158 )
   return 1;
  ...
  if (!sk_push(((_STACK*) (1 ? (ocerts) : (struct stack_st_X509*)0)), ((void*) (1 ? (x509) : (X509*)0)))) {
   X509_free(x509);
   return 0;
  }
