...
# 54 "modes/gcm128.c" 2
# 1 "modes/modes_lcl.h" 1
# 9 "modes/modes_lcl.h"
# 1 "../include/compat/machine/endian.h" 1
# 10 "modes/modes_lcl.h" 2

# 1 "../include/openssl/opensslconf.h" 1
# 1 "../include/openssl/opensslfeatures.h" 1
# 2 "../include/openssl/opensslconf.h" 2
# 12 "modes/modes_lcl.h" 2
...
# 68 "modes/modes_lcl.h"
typedef struct { u64 hi,lo; } u128;
# 79 "modes/modes_lcl.h"
...
# 282 "modes/gcm128.c"
 Htable[8] = V;
 do { if (sizeof(size_t)==8) { u64 T = 0xe100000000000000UL & (0-(V.lo&1)); V.lo = (V.hi<<63)|(V.lo>>1); V.hi = (V.hi>>1 )^T; } else { u32 T = 0xe1000000U & (0-(u32)(V.lo&1)); V.lo = (V.hi<<63)|(V.lo>>1); V.hi = (V.hi>>1 )^((u64)T<<32); } } while(0);
 Htable[4] = V;
 do { if (sizeof(size_t)==8) { u64 T = 0xe100000000000000UL & (0-(V.lo&1)); V.lo = (V.hi<<63)|(V.lo>>1); V.hi = (V.hi>>1 )^T; } else { u32 T = 0xe1000000U & (0-(u32)(V.lo&1)); V.lo = (V.hi<<63)|(V.lo>>1); V.hi = (V.hi>>1 )^((u64)T<<32); } } while(0);
 Htable[2] = V;
 do { if (sizeof(size_t)==8) { u64 T = 0xe100000000000000UL & (0-(V.lo&1)); V.lo = (V.hi<<63)|(V.lo>>1); V.hi = (V.hi>>1 )^T; } else { u32 T = 0xe1000000U & (0-(u32)(V.lo&1)); V.lo = (V.hi<<63)|(V.lo>>1); V.hi = (V.hi>>1 )^((u64)T<<32); } } while(0);
...
# 689 "modes/gcm128.c"
  ...
  ctx->H.u[0] = ({ u64 ret=(ctx->H.u[0]); asm ("bswapq %0" : "+r"(ret)); ret; });
  ctx->H.u[1] = ({ u64 ret=(ctx->H.u[1]); asm ("bswapq %0" : "+r"(ret)); ret; });
# 702 "modes/gcm128.c"
 }
 ...
 if (OPENSSL_ia32cap_P[0]&(1<<24) &&
     OPENSSL_ia32cap_P[1]&(1<<1) ) {
...
void CRYPTO_gcm128_setiv(GCM128_CONTEXT *ctx,const unsigned char *iv,size_t len)
{
 ...
 if (len==12) {
  ...
  while (len>=16) {
   ...
   (*gcm_gmult_p)(ctx->Yi.u,ctx->Htable);
   iv += 16;
   len -= 16;
  }
  if (len) {
...
# 785 "modes/gcm128.c"
   ...
   ctx->Yi.u[1] ^= ({ u64 ret=(len0); asm ("bswapq %0" : "+r"(ret)); ret; });
# 798 "modes/gcm128.c"
  }
  else
   ctx->Yi.u[1] ^= len0;
...
# 804 "modes/gcm128.c"
   ...
   ctr = ({ u32 ret=(ctx->Yi.d[3]); asm ("bswapl %0" : "+r"(ret)); ret; });
...
# 816 "modes/gcm128.c"
  ...
  ctx->Yi.d[3] = ({ u32 ret=(ctr); asm ("bswapl %0" : "+r"(ret)); ret; });
...
int CRYPTO_gcm128_aad(GCM128_CONTEXT *ctx,const unsigned char *aad,size_t len)
{
 ...
 alen += len;
 if (alen>(1UL<<61) || (sizeof(len)==8 && alen<len))
  return -1;
 ctx->len.u[0] = alen;
 ...
 if (n) {
  ...
  if (n==0) (*gcm_gmult_p)(ctx->Xi.u,ctx->Htable);
  else {
   ctx->ares = n;
   return 0;
  }
 ...
 if ((i = (len&(size_t)-16))) {
  (*gcm_ghash_p)(ctx->Xi.u,ctx->Htable,aad,i);
  aad += i;
  len -= i;
 }
# 874 "modes/gcm128.c"
...
# 949 "modes/gcm128.c"
   ...
   j -= 16;
      ...
      (*gcm_ghash_p)(ctx->Xi.u,ctx->Htable,out-(3*1024),(3*1024));
      len -= (3*1024);
  }
  if ((i = (len&(size_t)-16))) {
      size_t j=i;
...
# 975 "modes/gcm128.c"
   ...
   len -= 16;
      ...
      (*gcm_ghash_p)(ctx->Xi.u,ctx->Htable,out-j,j);
  }
# 1015 "modes/gcm128.c"
  if (len) {
   (*block)(ctx->Yi.c,ctx->EKi.c,key);
...
# 1087 "modes/gcm128.c"
 ...
 if (16%sizeof(size_t) == 0) do {
  ...
  while (len>=(3*1024)) {
      ...
      (*gcm_ghash_p)(ctx->Xi.u,ctx->Htable,in,(3*1024));
      while (j) {
       size_t *out_t=(size_t *)out;
       const size_t *in_t=(const size_t *)in;
...
# 1128 "modes/gcm128.c"
  ...
  if ((i = (len&(size_t)-16))) {
      (*gcm_ghash_p)(ctx->Xi.u,ctx->Htable,in,i);
      while (len>=16) {
       size_t *out_t=(size_t *)out;
       const size_t *in_t=(const size_t *)in;
...
# 1269 "modes/gcm128.c"
 ...
 while (len>=(3*1024)) {
  (*stream)(in,out,(3*1024)/16,key,ctx->Yi.c);
  ctr += (3*1024)/16;
  if (
# 1295 "modes/gcm128.c" 3 4
     1234 
# 1295 "modes/gcm128.c"
                == 
# 1295 "modes/gcm128.c" 3 4
...
# 1295 "modes/gcm128.c"
   ...
   ctx->Yi.d[3] = ctr;
  (*gcm_ghash_p)(ctx->Xi.u,ctx->Htable,out,(3*1024));
  out += (3*1024);
  in += (3*1024);
  len -= (3*1024);
 }
...
# 1314 "modes/gcm128.c"
  ...
  (*gcm_ghash_p)(ctx->Xi.u,ctx->Htable,out,i);
  out += i;
...
# 1489 "modes/gcm128.c"
  ...
  alen = ({ u64 ret=(alen); asm ("bswapq %0" : "+r"(ret)); ret; });
  clen = ({ u64 ret=(clen); asm ("bswapq %0" : "+r"(ret)); ret; });
# 1502 "modes/gcm128.c"
 }
