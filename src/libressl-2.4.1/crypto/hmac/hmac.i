...
# 79 "hmac/hmac.c"
  ...
  if ((size_t)j > sizeof(ctx->key)) {
   ERR_put_error(6,(174),(136),"hmac/hmac.c",83);
   goto err;
  }
  if (j < len) {
   ...
   if ((size_t)len > sizeof(ctx->key)) {
    ERR_put_error(6,(174),(137),"hmac/hmac.c",97)
                             ;
    goto err;
   }
...
HMAC_Init(HMAC_CTX *ctx, const void *key, int len, const EVP_MD *md)
{
 if (key && md)
  HMAC_CTX_init(ctx);
 return HMAC_Init_ex(ctx, key, len, md, 
# 135 "hmac/hmac.c" 3 4
                                       ((void *)0)
# 135 "hmac/hmac.c"
                                           );
