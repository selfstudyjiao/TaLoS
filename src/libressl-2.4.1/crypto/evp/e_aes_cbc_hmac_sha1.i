...
# 52 "evp/e_aes_cbc_hmac_sha1.c" 2
...
# 59 "evp/e_aes_cbc_hmac_sha1.c" 2
...
int AES_unwrap_key(AES_KEY *key, const unsigned char *iv, unsigned char *out,
    const unsigned char *in, unsigned int inlen);
# 61 "evp/e_aes_cbc_hmac_sha1.c" 2
...
# 62 "evp/e_aes_cbc_hmac_sha1.c" 2
# 1 "evp/evp_locl.h" 1
# 266 "evp/evp_locl.h"
...
# 63 "evp/e_aes_cbc_hmac_sha1.c" 2
# 1 "./constant_time_locl.h" 1
# 77 "./constant_time_locl.h"
...
# 64 "evp/e_aes_cbc_hmac_sha1.c" 2
# 73 "evp/e_aes_cbc_hmac_sha1.c"
typedef struct {
 AES_KEY ks;
...
sha1_update(SHA_CTX *c, const void *data, size_t len)
{
 ...
 if (len) {
  sha1_block_data_order(c, ptr, len / (16*4));
    ...
    const unsigned char *in, size_t len)
{
 ...
 if (ctx->encrypt) {
  if (plen == ((size_t)-1))
   plen = len;
  else if (len != ((plen + 20 + 16) &
      -16))
   return 0;
  else if (key->aux.tls_ver >= 0x0302)
   iv = 16;
  ...
  if (plen != len) {
   ...
   key->md = key->tail;
   sha1_update(&key->md, out + plen, 20);
   SHA1_Final(out + plen, &key->md);
  ...
  union {
   unsigned int u[20/sizeof(unsigned int)];
   unsigned char c[32 + 20];
  } mac, *pmac;
   ...
   *data = (void *)key->md.data;
   ...
   bitlen = ({ unsigned int r=(bitlen); asm ("bswapl %0":"=r"(r):"0"(r)); r; });
# 325 "evp/e_aes_cbc_hmac_sha1.c"
   pmac->u[0] = 0;
   pmac->u[1] = 0;
   pmac->u[2] = 0;
   pmac->u[3] = 0;
   ...
   pmac->u[0] = ({ unsigned int r=(pmac->u[0]); asm ("bswapl %0":"=r"(r):"0"(r)); r; });
   pmac->u[1] = ({ unsigned int r=(pmac->u[1]); asm ("bswapl %0":"=r"(r):"0"(r)); r; });
   pmac->u[2] = ({ unsigned int r=(pmac->u[2]); asm ("bswapl %0":"=r"(r):"0"(r)); r; });
   pmac->u[3] = ({ unsigned int r=(pmac->u[3]); asm ("bswapl %0":"=r"(r):"0"(r)); r; });
   pmac->u[4] = ({ unsigned int r=(pmac->u[4]); asm ("bswapl %0":"=r"(r):"0"(r)); r; });
# 400 "evp/e_aes_cbc_hmac_sha1.c"
   len += 20;
# 422 "evp/e_aes_cbc_hmac_sha1.c"
   key->md = key->tail;
   sha1_update(&key->md, pmac->c, 20);
...
# 582 "evp/e_aes_cbc_hmac_sha1.c" 3 4
                                      ((void *)0)
# 582 "evp/e_aes_cbc_hmac_sha1.c"
                                          ;
}
...
# 589 "evp/e_aes_cbc_hmac_sha1.c" 3 4
                                      ((void *)0)
# 589 "evp/e_aes_cbc_hmac_sha1.c"
                                          ;
}
